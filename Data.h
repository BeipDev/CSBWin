...
public:
   ...
   FILL(22576, 22572)
   RecordName MonsterUnderAttack{}; // 22572
   ...
   i8 Byte16702{};
   FILL(16701, 16634)
   RecordName objOpenChest{};           // 16634; The one opened and on display
   RecordName rnChestContents[8]{};     // 16632;
   ...
   RecordName objectInHand{}; // 16576; //object in hand cursor?
   ...
   RecordName LastMonsterMoved{};   // Obj13020; //Any timer 29 to 41.
...
public:
   ...
   i8 Byte10638{};
   FILL(10637, 10574)
   RecordName rn10574[5]{};               // Is RN right??? Is 5 right???
   BUTTON_DEF ViewportObjectButtons[6]{}; // 10564;
                                          //  DrawViewport constructs a button for
                                          //  each object that can be clicked.
   RecordName unused10540{};              // rn10540; //Replaced by entries in SUUMARIZEROOMDATA
   ...
   std::vector<RecordName> objectList; // prn10464;
...
class GameTimers
{
   ...
   std::vector<HTIMER> m_timerQueue;
   std::vector<HTIMER> m_searchList;
...
private:
   ...
   ui16 *TimerQueue() { return m_timerQueue.data(); } // Point to first word of timer queue
...
public:
   ...
   {
      m_index = -1;
      m_numEntry = gameTimers.CreateSearchList();
   }
   ~TIMER_SEARCH() { gameTimers.m_searchActive = false; }
   bool FindNextTimer();
   TIMER *TimerAddr() const { return gameTimers.pTimer(gameTimers.m_searchList[m_index]); }
   HTIMER TimerHandle() const { return gameTimers.m_searchList[m_index]; }
...
public:                                      // DSA
   ...
   {
      return abs(r) < MAXSTATE;
   }; // DSA
   void SetMaxState() { nextState = MAXSTATE; }; // DSA
   DSACOMMAND CmdCode()                          // DSA
   {
      return (DSACOMMAND)(cmdCode = COMMANDCODE);
   }; // DSA
}; // DSA
   // DSA
