...
PALETTE palette24862 = {
    0x0000, 0x0333, 0x0444, 0x0310, 0x0066, 0x0420, 0x0040, 0x0060, 0x0700, 0x0750, 0x0643, 0x0770, 0x0222, 0x0555, 0x0007, 0x0777};
...
{ //()
   ...
   if(LoadPnt((ui8 *)&e.Pnt24812) == NULL)
      RETURN;
   if(e.Word24814 == 0)
      RETURN;
...
i16 TAG00142e(pnt P1)
{
   ...
   A0 = (aReg)LoadPnt(A4 + 690);
   {
      ...
      if(cellflagArraySize != 0)
         cellArraySize = cellflagArraySize;
...
void TAG001676(ui8 *P1, i16 P2)
{
   ...
   if(LoadPnt(A4 + 678) != NULL)
      ReleaseMemory(LoadPnt(A4 + 678));
   StorePnt(A4 + 678, (ui8 *)NULL);
   if(LoadPnt(A4 + 682) != NULL)
      ReleaseMemory(LoadPnt(A4 + 682));
   StorePnt(A4 + 682, (ui8 *)NULL);
   if(LoadPnt(A4 + 686) != NULL)
      ReleaseMemory(LoadPnt(A4 + 686));
   StorePnt(A4 + 686, (ui8 *)NULL);
   if(LoadPnt(A4 + 690) != NULL)
      ReleaseMemory(LoadPnt(A4 + 690));
   StorePnt(A4 + 690, (ui8 *)NULL);
   if(LoadPnt(A4 + 694) != NULL)
      ReleaseMemory(LoadPnt(A4 + 694));
   StorePnt(A4 + 694, (ui8 *)NULL);
   if(LoadPnt(A4 + 698) != NULL)
      ReleaseMemory(LoadPnt(A4 + 698));
   StorePnt(A4 + 698, (ui8 *)NULL);
   if(LoadPnt(A4 + 704) != NULL)
      ReleaseMemory(LoadPnt(A4 + 704));
   StorePnt(A4 + 704, (ui8 *)NULL);
   for(D7W = 0; D7W < 16; D7W++)
   {
      A0 = A4 + 708 + 4 * D7W;
      if(LoadPnt(A0) != NULL)
         ReleaseMemory(LoadPnt(A0));
...
{ //(i16)
   ...
   for(;;)
   {
      if((wordGear(LoadPnt((ui8 *)&e.Pnt24812)) == 0) || (e.Word24814 == 0))
      {
      ...
      else
      {
         ...
         {
            if((pBlock1[301] != 1) && (LE16(wordGear(pBlock1 + 376)) != 1))
               break; // Word22594
            if(LE16(wordGear(pBlock1 + 378)) > 13)
               break; // Word22592
         ...
         else
         {
            if(pnt_4[337] > 1)
            {
               if(wordGear(pnt_4 + 412) != 1)
                  break;
               if(wordGear(pnt_4 + 414) > 12)
                  break;
            }
         }
         CheckChampionNames(_1_);
...
{ //(i16)
   ...
   pnt_4 = P1 + 38;
   if((wordGear(P1 + 2) == 2) && ((UI8)(pnt_4[301]) >= 2) && (LE16(wordGear(pnt_4 + 378)) == 13))
      RETURN_i16(1);
...
void EnqueMouseClick(i32 P1, i32 P2, i32 P3)
{
   ...
   if(D5W > 4)
      D5W -= 5;
   if(D5W == e.FirstQueuedMouseEvent)
      return;
...
{ //(i16)
   ...
   for(;;)
   {
      ...
      if(pressed_key)
         EditCharacterName((char)pressed_key);
   }
   e.Byte9212 = -128;
...
ui8 *TAG0073d4(pnt P1)
{
       ...
       D7W++, A3 += 18)
   {
      ...
      {
         if((ui8 *)A4 <= LoadPnt(A3) + LoadLong(A3 + 6))
            break;
      }
      //
      //
...
char *TAG007ac0(pnt *P1)
{
   ...
   {
      if(wordGear(A2 + 12) >= wordGear(A2 + 10))
         continue;
      D6W = wordGear(A2 + 14);
      D0L = (D6UW << 2) & 0xffff;
      A3 = A2 + D0L + 16;
...
ui8 *TAG007d4c(STRUCT12 *P1)
{
   ...
   if((pnt_4 != NULL) && ((A3 = (aReg)TAG007cc6(D7L, s12A4->word4, pnt_4)) != NULL))
   {
      ...
      if((s12A4->fnc8 != NULL) && (s12A4->word6 != 0))
      {
         (*s12A4->fnc8)(A3);
      }
