...
public:
   ui32 m_priority;
   void Get(i32 *level, i32 *x, i32 *y, RecordName *obj);
   SDOBJ();
   SDOBJ(i32 level, i32 x, i32 y, RecordName *obj, ui32 priority);
};

SDOBJ::SDOBJ(i32 level, i32 x, i32 y, RecordName *obj, ui32 priority)
{
...
void SDOBJ::Get(i32 *level, i32 *x, i32 *y, RecordName *obj)
{
...
{ //
   ...
   while(index != 0)
   {
      ...
      if(m_sdobj[parent].m_priority >= pSDobj->m_priority)
         break;
...
{ // Deletes the topmost object
   ...
   while(index != 0)
   {
      ...
      //    parentPriority = ComputePriority(&m_sdobj[parent]);
      //    if (parentPriority >= priority) break;
      if(m_sdobj[parent].m_priority <= priority)
         break;
      m_sdobj[index] = m_sdobj[parent];
      index = parent;
   }
      ...
      switch(currentDBNum)
      {
         case dbCLOTHING:
            ...
            }
            {
               ...
               if(isTriggered)
               {
                  if(pileSize[idx][pos] == 1)
                     break;
               }
               if(pClothingDB->important())
                  break;
...
static int ScanDungeon()
{
   ...
   if(currentX >= levelWidth)
   {
      ...
      currentLevel++;
      if(m_trace != NULL)
         fprintf(m_trace, "set level to %02x\n", currentLevel);
      return 0;
   }
   ...
   if(currentY >= levelHeight)
   {
      ...
      currentY = 0;
      if(m_trace != NULL)
         fprintf(m_trace, "set X to %02x\n", currentX);
      return 0;
   }
   result = ScanOneCell(&objlist);
   currentY++;
   if(m_trace != NULL)
      fprintf(m_trace, "SmartDiscard set Y to %02x\n", currentY);
   return result;
}
...
void SmartDiscard(bool Initialize)
{
   ...
   switch(currentMode)
   {
         ...
         for(repeat = 0; repeat < 50; repeat++)
         {
            ...
            switch(currentDBNum)
            {
               ...
               case dbCLOTHING: {
                  ...
                  if(m_trace != NULL)
                     fprintf(m_trace, "count CLOTHING ");
               }
               break;
               default:
      ...
      case ModeDiscard:
         ...
         DiscardObject();
         if(m_trace != NULL)
            fprintf(m_trace, "Discard an object\n");
         return;
   }
