...
void SetCursed(RecordName obj, bool curse);
...
void INSTRUMENTATION::Dump(FILE *f)
{
   ...
   for(absDSAIndex = 0; absDSAIndex < 256; absDSAIndex++)
   {
      ...
      if(pMaster == NULL)
         continue;
      fprintf(f, " %03d %8d %8d %-80.80s\n", absDSAIndex, m_DSACounts[absDSAIndex], m_DSAOperations[absDSAIndex], pMaster->Description());
...
struct DSADBANK
{
   ...
   i32 Var(i32 i)
   {
      if(pDsaVars->definedFlags[i] != DVT_WellDefined)
         NoValue(i);
      return pDsaVars->values[i];
   }
   void Var(i32 i, i32 value)
   {
      pDsaVars->values[i] = value;
      pDsaVars->definedFlags[i] = (ui8)DVT_WellDefined;
   }
   void SetVarBit(i32 i, i32 bitmask)
   {
      if(pDsaVars->definedFlags[i] != DVT_WellDefined)
         NoValue(i);
      pDsaVars->values[i] |= bitmask;
   }
   void SetVarState(i32 i, DSAVAR_TYPE dvt) { pDsaVars->definedFlags[i] = (ui8)dvt; };
   bool ConsumateDeletes(RecordName RNslave);
   void DeleteObject(RecordName obj);
   DSAVAR_TYPE GetVarState(i32 i) { return (DSAVAR_TYPE)pDsaVars->definedFlags[i]; };
   DSADBANK()
   {
      numText = 0;
      mostRecentInterestingObjectID = -1;
      m_objectDeleteList = RNeof;
   }
   ~DSADBANK();
};
...
void DSADBANK::DeleteObject(RecordName obj)
{
   DBCOMMON *pDB;
   ...
   if(numText != 0)
   {
      for(i = 0; i < 10; i++)
      {
         if(textStrings[i] != NULL)
            UI_free(textStrings[i]);
      }
   }
}

bool DSADBANK::ConsumateDeletes(RecordName RNslave)
{
   bool result = false;
...
struct EXECUTIONPACKET
{
   ...
   RecordName m_RNmaster;    // Unchanged
   RecordName m_RNslave;     // Unchanged
};
...
static char *constructDebugMessage(const char *msg)
{
   ...
   sprintf(debugMessage, "%s\nDSA at %d(%d,%d)\nRow=%d;Col=%d", msg, pExPkt->m_locrMaster.l, pExPkt->m_locrMaster.x, pExPkt->m_locrMaster.y, pExPkt->m_curState, pExPkt->m_column);
   return debugMessage;
}
...
{
   if(l >= d.dungeonDatIndex->NumLevel())
      return false;
   if(x > d.pLevelDescriptors[l].LastColumn())
      return false;
   if(y > d.pLevelDescriptors[l].LastRow())
      return false;
   if(x < 0)
      return false;
   if(y < 0)
...
static i32 Execute(RecordName RNmaster,
                   RecordName RNslave,
                   DSA *pMaster,
                   DSA *pSlave,
                   i32 curState,
...
static RecordName FindMaster(DSA *pSlave, RecordName objSlave, i32 x, i32 y,
...
static void EX_JUMP(EXECUTIONPACKET &exPkt)
{
   ...
   if(pdsaDbank->tracing)
   {
      fprintf(GETFILE(TraceFile), " Jump %d%s", exPkt.m_transferState, colNames[exPkt.m_transferCol]);
   }
}
...
static void EX_QUESTION(EXECUTIONPACKET &exPkt)
{
   ...
   switch(cCmd)
   {
      case 1:
         ...
         if(pdsaDbank->tracing)
         {
            fprintf(GETFILE(TraceFile), " Jump %d%s", addr, colNames[cCol]);
         }
         return;
      case 2:
         // Gosub
...
static void EX_CASE(EXECUTIONPACKET &exPkt)
{
   ...
   for(n = numCase; n > 0;)
   {
      ...
      if(pCase[2 * m] == value)
      {
         ...
         if(pdsaDbank->tracing)
         {
            fprintf(GETFILE(TraceFile), " Jump %d%s", addr, colNames[col]);
         }
         break;
      }
      if(pCase[2 * m] > value)
   ...
   switch(where)
   {
      ...
      case LOAD_ABS:
         if(pdsaDbank->tracing)
         {
            ...
            loca = locr;
            fprintf(GETFILE(TraceFile), "ABSLOCATION %d(%d,%d)%d", loca.l, loca.x, loca.y, loca.p);
         }
         pdsaDbank->stack.push(*(exPkt.m_PC++));
         exPkt.m_wordsRemaining--;
         break;
      ...
      default:
         ...
         while(where > 1)
         {
            ...
            if(RNparameter == RNeof)
               break;
            if(RNparameter.dbType() != dbACTUATOR)
               continue;
            pParameter = GetRecordAddressDB3(RNparameter);
            if(pParameter->actuatorType() != 47)
               continue;
            where -= 2;
         }
         if(RNparameter == RNeof)
...
static void EX_EQUAL(EXECUTIONPACKET &exPkt)
{
   ...
   if(pdsaDbank->tracing)
   {
      fprintf(GETFILE(TraceFile), "(%d,%d) --> %d", v1, v2, v1 == v2 ? 1 : 0);
   }
...
bool IsCarriedCallback(RecordName thisObj, int srcObj)
{
...
bool ForEach(int thisObj, int srcObj, bool callback(RecordName thisObj, int srcObj))
{
   // return true to terminate search.
   RecordName obj;
   int i, chIdx;
...
static void EX_IsCarried()
{
   ...
   {
      if((charnum == chidx) || ((charnum == 4) && (chidx == d.HandChar)) || (charnum == 5))
      {
...
static void EX_DEL()
{
   ...
   if(iLocation < 0)
   {
      ...
      else
      {
         ...
         if(posIdx >= 30)
            return;
         if(d.CH16482[chIdx].Possession(posIdx) == RNnul)
            return;
   ...
   else
   {
      ...
      {
         if(thisObj.ConvertToInteger() != objID)
            continue;
...
static RecordName CopyItemList(RecordName origRN);
static RecordName CopyItem(RecordName origRN)
{
   DBTYPE dbType;
   RecordName CopyRN = RNeof;
   RecordName copiedPossessions;
   if(origRN == RNeof)
      return CopyRN;
   ...
   switch(dbType)
   {
      ...
      case dbMONSTER: {
         ...
         if((d.ITEM16QueLen >= d.MaxITEM16 - 5) && (d.LoadedLevel == d.partyLevel))
            break;
         ...
         copiedPossessions = CopyItemList(pDB4->possession());
         newPDB4 = GetRecordAddressDB4(CopyRN);
         newPDB4->possession(copiedPossessions);
         newPDB4->groupIndex(0xd8);
         if(monsterSize == 2)
            newPDB4->groupIndex(0xff);
...
static RecordName CopyItemList(RecordName origRN)
{
       ...
       origRN = GetDBRecordLink(origRN))
   {
      ...
      if(result == RNeof)
         ...
         db.GetCommonAddress(lastItem)->link(newItem);
      lastItem = newItem;
   }
   return result;
}
...
{
   LOCATIONREL locr;
   RecordName object, newObj, RNmon;
   ui32 objID;
   i32 iLocation;
   ui32 posMsk;
   i32 oldLevel;
   ...
   {
      locr.Integer(iLocation);
      if(locr.l >= d.dungeonDatIndex->NumLevel())
         return;
      if(locr.x > d.pLevelDescriptors[locr.l].LastColumn())
         return;
      if(locr.y > d.pLevelDescriptors[locr.l].LastRow())
         return;
   }
         ...
         else // must be a monster
         {
            ...
            if(*pDest == RNeof)
               *pDest = newObj;
               ...
               AddObjectToRoom(newObj, *pDest, -1, 0, NULL);
         }
      }
   }
   return;
...
{
   if((d.partyLevel == locr.l) && (d.partyX == locr.x) && (d.partyY == locr.y))
      return 1 << 29;
   if(FindFirstMonster(locr.l, locr.x, locr.y) != RNeof)
      return 1 << 30;
   return 0x80000000;
}
...
static void ExamineCell(ui32 first, ui32 last, const char *trc)
{
   ...
   for(i = first; i <= last; i++)
   {
      ...
      if(((criteriaMask & cellBits & 0x01ffffff) != 0) && ((criteriaMask & cellBits & 0xe0000000) != 0))
      {
         result |= resultbit;
      }
      resultbit <<= 1;
   ...
   switch(subcode)
   {
      ...
      case STKOP_CreateCloud: {
         ...
         if(spellObj != RNeof)
         {
            ...
            {
               CreateCloud(spellObj, (i16)size, (i16)locr.x, (i16)locr.y, (i16)locr.x, (i16)locr.y, (i16)locr.p, NULL);
            }
            if(oldLevel != d.LoadedLevel)
               LoadLevel(oldLevel);
         }
      }
      break;
      ...
      case STKOP_MissileInfoStore: {
         ...
         if(obj.checkIndirectIndex(k))
         {
            ...
            if(obj.dbType() == dbMISSILE)
            {
               ...
               hTimer = pMissile->timerIndex();
               gameTimers.pTimer(hTimer)->timerWord8() = (i16)((gameTimers.pTimer(hTimer)->timerWord8() & (~(0x3 << 10))) | ((direction & 3) << 10));
            }
         }
      }
      break;
      ...
      case STKOP_FalsePit: //(target bool ...)
      {
         ...
         locr.Integer(W);
         if(!locr.IsValid())
            break; // if bad location
         pcf = GetCellFlagsAddress(locr.l, locr.x, locr.y);
         if(RoomType(*pcf) != roomPIT)
            break;
         if(V != 0)
            *pcf |= 1;
         else
      ...
      case STKOP_ParamStore: {
         ...
         else
         {
            ...
            for(j = 0; j < n; j++)
            {
               if(pdsaDbank->GetVarState(i + j) != DVT_NonParameter)
                  pDSAparameters[1 + j] = pdsaDbank->Var(i + j);
            }
         }
         if(pdsaDbank->tracing)
         {
      ...
      case STKOP_TextFetch: {
         ...
         if(textIndex > 9)
            break;
         if(!RNtext.checkIndirectIndex(objIndex))
            break;
         RNtext.ConstructFromInteger(objIndex);
         if(RNtext == RNeof)
            break;
         if(RNtext.dbNum() != dbTEXT)
            break;
      ...
      case STKOP_ChPoss: {
         ...
         if(object == RNnul)
            ...
            objIndex = object.ConvertToInteger();
      ...
      case STKOP_CellStore: {
         ...
         if(arraySize == 0)
            break;
         if(pdsaDbank->Var(arrayIndex + 0) != rt)
            break;
         if(arraySize < 2)
            break;
         switch(rt)
         {
            case roomSTONE: {
               cf &= 0xe0;
               if(pdsaDbank->Var(arrayIndex + 1) & 1)
                  cf |= 0x8;
               if(pdsaDbank->Var(arrayIndex + 1) & 2)
                  cf |= 0x4;
               if(pdsaDbank->Var(arrayIndex + 1) & 4)
                  cf |= 0x2;
               if(pdsaDbank->Var(arrayIndex + 1) & 8)
                  cf |= 0x1;
            }
            break;
            case roomOPEN: {
            ...
            case roomDOOR: {
               ...
               switch(oldState)
               {
                  case 5: // if it is bashed
                     if(newState == 0)
                        oldState = newState; // set open
                     break;
                  case 4: // if it is closed
                     if(newState == 5)
                        oldState = newState; // set bashed
                     break;
      ...
      case STKOP_CharFetch: {
         ...
         if(charIdx == 4)
            charIdx = d.HandChar;
         if((index + num > 100) || ((charIdx < 0x10000) && (charIdx >= d.NumCharacter)) || (charIdx < 0))
         {
            memset(result, 0, sizeof(result));
         }
         else
         {
            ...
            for(i = 0; i < 7; i++)
            {
               result[10 + i] = (pc->Attributes[i].Minimum() << 16) | (pc->Attributes[i].Maximum() << 8) | (pc->Attributes[i].Current() << 0);
            }
            for(i = 0; i < 40; i += 2)
            {
      ...
      case STKOP_TalentsFetch: {
         ...
         if(charIdx == 4)
            charIdx = d.HandChar;
         if(
             ((charIdx < 0x10000) && (charIdx >= d.NumCharacter)) || (charIdx < 0))
         {
            pc = NULL;
         }
         else
      ...
      case STKOP_CharStore: {
         ...
         if(charIdx < 0)
            break;
         if(d.PotentialCharacterOrdinal == charIdx + 1)
            break;
         pc = d.CH16482 + charIdx;
         n = 59;
         if(num < n)
         ...
         if(pc->HP() - d.PendingDamage[charIdx] > 0) // Nothing we can do about dead heroes!
         {
            if(n > 2)
            {
               ...
               if(pdsaDbank->Var(2 + index) < 0)
                  pdsaDbank->Var(2 + index, 0);
      ...
      case STKOP_TeleportParty: {
         ...
         oldLevel = d.LoadedLevel;
         if(oldLevel != d.partyLevel)
            LoadLevel(d.partyLevel);
         MoveObject(RNnul, // We wiggle the party and pretend a
                    d.partyX,
                    d.partyY, // teleporter is present!
                    d.partyX,
                    d.partyY,
                    &pretendTeleporter,
                    NULL);
      ...
      case STKOP_Move: {
         ...
         NOFILTER
         result = MoveObject(srcType, srcObjMsk, srcPosMsk, srcLoc, srcDepth, dstType, dstObjMsk, dstPosMsk, dstLoc, dstDepth);
      ...
      case STKOP_GeneratorDelayFetch: {
         ...
         if(rnGenerator == RNeof)
            pdsaDbank->stack.push(-1);
         else
            pdsaDbank->stack.push(GetRecordAddressDB3(rnGenerator)->disableTime());
      }
      break;
      case STKOP_JumpGear:
         ...
         if(pdsaDbank->tracing)
         {
            fprintf(GETFILE(TraceFile), "J* %d%s", exPkt.m_transferState, colNames[exPkt.m_transferCol]);
         }
         break;
      ...
      case STKOP_MonLandD: {
         ...
         distance = abs((int)pDSAparameters[1 + 1] - (int)pDSAparameters[1 + 5]) + abs((int)pDSAparameters[1 + 2] - (int)pDSAparameters[1 + 6]);
      ...
      case STKOP_CountInjury: {
         ...
         for(chidx = 0; chidx < d.NumCharacter; chidx++, chMask >>= 1)
         {
            ...
            count += bitCounts[injuries & 15] + bitCounts[(injuries >> 4) & 15] + bitCounts[(injuries >> 8) & 15] + bitCounts[(injuries >> 12) & 15];
         }
      ...
      case STKOP_BitCount: {
         ...
         V = pdsaDbank->stack.pop();
         pdsaDbank->stack.push(bitCounts[(V >> 0) & 0x0f] + bitCounts[(V >> 4) & 0x0f] + bitCounts[(V >> 8) & 0x0f] + bitCounts[(V >> 12) & 0x0f] + bitCounts[(V >> 16) & 0x0f] + bitCounts[(V >> 20) & 0x0f] + bitCounts[(V >> 24) & 0x0f] + bitCounts[(V >> 28) & 0x0f]);
      }
      break;
      case STKOP_VSET: {
         ui32 n, dst, i;
...
static void EX_FETCH(EXECUTIONPACKET &exPkt)
{
   ...
   for(thisObj = FindFirstObject(locr.l, locr.x, locr.y);
       ...
       thisObj = GetDBRecordLink(thisObj))
   {
      if(((1 << thisObj.pos()) & positionMask) == 0)
         continue;
      if(((1 << thisObj.dbType()) & objMask) == 0)
         continue;
      if(depth == 0)
      {
                   ...
                   bool filter)
{
   ...
   if(pdsaDbank->tracing)
   {
      ...
      fprintf(GETFILE(TraceFile),
              ...
              locaSlave.x,
              locaSlave.y,
              exPkt.m_locrMaster.l,
              exPkt.m_locrMaster.x,
              exPkt.m_locrMaster.y,
              exPkt.m_curState,
              msgType);
   }
   do
   {
      ...
      while(exPkt.m_wordsRemaining > 0)
      {
         ...
         if(exPkt.m_transferState >= 0)
            break; // Jump command
         if((finalState == -1) && (exPkt.m_nextState >= 0))
         {
...
bool ProcessDSATimer5(RecordName objSlave,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave)
{
   DSAVARS dsaVars;
...
bool ProcessDSATimer7(RecordName objSlave,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave)
{
   DSAVARS dsaVars;
...
void ProcessDSAFilter(RecordName objSlave,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave,
                      bool *pFilterActive,
                      DSAVARS *dsaVars)
...
bool ProcessDSATimer6(RecordName objSlave,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave,
                      bool filter,
                      DSAVARS *dsaVars)
{
   ...
   //  EXECUTIONPACKET exPkt;
   //  SAVEPEXPKT save(&exPkt);
   RecordName objMaster;
   DB3 *pDBSlave, *pDBMaster;
   DSA *pSlave, *pMaster;
   DSADBANK dsaDbank;
   DSADBANK *saveDsaDbank;
   if(timerTypeModifier[pTimer->timerUByte9()] == 3)
      return false;
   dsaActive = true;
   ...
   objMaster = FindMaster(pSlave, objSlave, timerX, timerY, &masterX, &masterY, &masterPos);
   ...
   else
   {
      if(TimerTraceActive)
      {
         fprintf(GETFILE(TraceFile), "DSA %s State %d MSG %d [%d]\n", pSlave->Description(), curState, inputMsgType, operationCount - startingOperationCount);
      }
   }
   if(pdsaDbank->forceState >= 0)
      curState = pdsaDbank->forceState;
   PutState(GetRecordAddressDB3(objMaster), pMaster, curState);
   if(!excessiveWarning)
   {
      if(operationCount > 50000)
      {
         if((d.Time < prevTime + 6) || ((6 * operationCount) > 50000 * (d.Time - prevTime)))
         {
            if(!IsPlayFileOpen())
            {
