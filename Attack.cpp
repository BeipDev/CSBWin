...
//*********************************************************
// TAG00f0e8
RecordName IsLordChaosHere(i32 mapX, i32 mapY)
{
   RecordName objD7;
   DB4 *DB4A3;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   if(DB4A3->monsterType() != mon_LordChaos)
      return RNempty;
   return objD7;
}
...
{
   if(FindFirstMonster(x, y) != RNeof)
      return true;
   if((d.LoadedLevel == d.partyLevel) && (d.partyX == x) && (d.partyY == y))
      return true;
   return false;
}
...
{ //()
   ...
   if(D6W >= d.height)
      RETURN;
   CreateCloud(RecordName(RNDispellMissile), 0xff, D7W, D6W, D7W, D6W, 0xff, NULL);
...
#define NewFusion
#ifdef NewFusion
   for(i = 0; i < 4; i++)
      direction[i] = i; // Initially north, east, south, west
   ...
   for(i = 0; i < 4; i++) // for each direction in scrambled order
   {
      ...
      fluxCageOrWall[i] = IsCellFluxcage(x1[i], y1[i]) || IsCellWall(x1[i], y1[i]);
      empty1[i] = IsCellOpenOrPit(x1[i], y1[i]) && !IsCellOccupied(x1[i], y1[i]);
      empty2[i] = legalXY(x2[i], y2[i]) && IsCellOpenOrPit(x2[i], y2[i]) && !IsCellOccupied(x2[i], y2[i]) && !IsCellFluxcage(x2[i], y2[i]);
   }
...
void PhysicalAttackFilter(ATTACKPARAMETERS *pParam, FILTER *pFilter, const char *traceID)
{
   ...
   if(pParam->monsterUnderAttack == 0)
      return;
   ...
   if(traceID != NULL)
   {
      fprintf(GETFILE(TraceFile), "%sw_2 = DamageMonster() --> %d\n", traceID, pParam->attdep.physicalAttack.monsterDamage);
   }
    ...
    const char *traceID)
{
   ...
   if((!pmtDesc->nonMaterial()) || vorpalOrDisrupt)
   {
      if(traceID != NULL)
      {
      ...
      if(D7W != 0)
         D7W = sw(D7W + (ranResult = (STRandom() & 0xffff) % D7W));
      if(traceID != NULL)
      {
      ...
      if(traceID != NULL)
      {
         if((objNI_attackWeapon == objNI_VorpalBlade) && !pmtDesc->nonMaterial())
         {
            fprintf(GETFILE(TraceFile), "%sDivide D7W by 2 because VorpalBlade and i26 Word2 bit 6 = 0 --> %d\n", traceID, D7W / 2);
         }
      }
      if((objNI_attackWeapon == objNI_VorpalBlade) && (!pmtDesc->nonMaterial()) && ((D7W = sw(D7W / 2)) == 0))
      {
         if(traceID != NULL)
         {
...
{ //(i16)
   ...
   pParam->attdep.physicalAttack.attackedMonsterOrdinal = DetermineAttackOrdinal(
       pParam->attackX, pParam->attackY, d.partyX, d.partyY, d.CH16482[pParam->charIdx].charPosition);
   ...
   if((pParam->attdep.warcryetc.requiredMastery > pParam->attdep.warcryetc.effectiveMastery) || (pParam->attdep.warcryetc.requiredMastery == 15))
   {
      ...
      if(AttackTraceActive)
      {
         ...
         if(pParam->attdep.warcryetc.requiredMastery > pParam->attdep.warcryetc.effectiveMastery)
            ...
            fprintf(GETFILE(TraceFile), "requiredMastery == 15\n");
      }
   }
   else
   {
      ...
      D1L = 0;
      D1B = pmtDesc->timePerMove(); // uByte6;
      D0W = sw(D0L / D1W);
      pi16_4->uByte5 = D0B;
      success = 1;
   }
...
void FluxCage(i32 mapX, i32 mapY, ATTACKPARAMETERS *pParam)
{
   ...
   if((rt != roomSTONE) && (rt != roomSTAIRS))
   {
      ...
      if(rnD5 != RNnul)
      {
         ...
         //      };
         if((IsLordChaosHere(LCX = mapX, LCY = mapY - 1) != RNempty) || (IsLordChaosHere(LCX = mapX + 1, LCY = mapY) != RNempty) || (IsLordChaosHere(LCX = mapX, LCY = mapY + 1) != RNempty) || (IsLordChaosHere(LCX = mapX - 1, LCY = mapY) != RNempty))
         {
            cageCount = IsCellFluxcage(LCX, LCY - 1) + IsCellFluxcage(LCX + 1, LCY) + IsCellFluxcage(LCX, LCY - 1) + IsCellFluxcage(LCX - 1, LCY);
            if(cageCount == 3)
            {
               pParam->activateMonster = 1;
               pParam->attackX = LCX;
...
{ //(i16)
   ...
   if((pParam->skillNumber == 16) // All the Wizard skills
      || (pParam->skillNumber == 17) || (pParam->skillNumber == 18) || (pParam->skillNumber == 19) || (pParam->skillNumber == 3))
   {
   ...
   switch(pParam->attackType)
   {
      ...
      case atk_SHOOT: // PAF
         ...
         if((pChar->Possession(0) == RNnul) || (pChar->Possession(0).dbType() != dbWEAPON))
         {
      ...
      case atk_HEAL: // PAF
         ...
         if(neededHP_4 > 0)
         {
            if(pChar->Mana() != 0)
            {
               ...
               pParam->attdep.heal.HPIncrement = sw(Smaller(10, DetermineMastery(pParam->charIdx, 13)));
               pParam->experienceGained = 2;
               CallAttackFilter(&filter, pParam, 1);
               do
               {
      ...
      case atk_CLIMBDOWN: // PAF
         ...
         if((rtD0 == roomPIT) && (FindFirstMonster(pParam->attackX, pParam->attackY) == RNeof))
         {
            d.SupressPitDamage = 1;
            MoveObject(RecordName(RNnul), d.partyX, d.partyY, pParam->attackX, pParam->attackY, NULL, NULL);
      ...
      case atk_FREEZELIFE: // PAF
         ...
         if(dbA2->IsDBType(dbMISC))
         {
            ...
            if(DB10A2->miscType() == misc_MagicalBoxA)
               D4W = 30;
            if(DB10A2->miscType() == misc_MagicalBoxB)
               D4W = 125;
         }
         if(D4W != 0)
         {
         ...
         if(d.freezeLifeTimer + D4W > 200)
            D4L = 200 - d.freezeLifeTimer;
         if(D4W < 0)
            D4W = 0;
      ...
      case atk_THROW: // PAF
         SetCharToPartyFacing(pChar);
         if((pChar->charPosition == ((d.partyFacing + 1) & 3)) || (pChar->charPosition == ((d.partyFacing + 2) & 3)))
         {
            D0W = 1; // right side
         }
         else
         ...
         {
            successfulAttack = ThrowByCharacter(pParam->charIdx, 1, D0W, pParam->disableTime, pParam->attdep.thro.range, pParam->attdep.thro.damage, pParam->attdep.thro.decayRate);
         ...
         if(successfulAttack != 0)
         {
            ...
            if(pParam->disableTime > 255)
               pParam->disableTime = 255;
   ...
   if(TimerTraceActive)
   {
      fprintf(GETFILE(TraceFile), " Increase skill %02x by %02x to %02x\n", pParam->skillNumber, pParam->experienceGained, pChar->skills92[pParam->skillNumber].experience);
   }
   DrawCharacterState(pParam->charIdx);
...
{ //()    Called by Fusion attack only.
   ...
   static DB4 *DB4A3;
   static i8 b_226[200];
   static RecordName obj_26[8];
   static i8 b_9;
   static RecordName obj_8;
   static i16 w_6;
   static i16 w_4;
   static i16 i;
   static i16 x, y;
   ...
   for(i = 0, x = d.partyX, y = d.partyY; i < 2; x = w_4, y = w_6, i++)
   {
      do
      {
         ...
         {
            if(objD4.dbType() != dbCLOUD)
               continue;
            DB15A2 = GetRecordAddressDB15(objD4);
            if(DB15A2->cloudType() != CT_FLUXCAGE)
               continue;
   ...
   {
      CreateCloud(RecordName(RNFireball), D7w, w_4, w_6, w_4, w_6, 255, NULL);
      ProcessTimersViewportAndSound(_4_); // TAG01fed6
      // if (testdelay) pauseAwhile(testdelay);
      if(testdelay)
         VBLDelay(_2_, MS2VBL(testdelay));
      //
      //
   }
   ...
   // if (testdelay) pauseAwhile(testdelay);
   if(testdelay)
      VBLDelay(_6_, MS2VBL(testdelay));
   for(D7w = 55; D7w <= 255; D7w += 40)
   {
      CreateCloud(RecordName(RNDispellMissile), D7w, w_4, w_6, w_4, w_6, 255, NULL);
      ProcessTimersViewportAndSound(_7_); // TAG01fed6(_7_);
      if(testdelay)
         VBLDelay(_9_, MS2VBL(testdelay));
      //
      //
   }
   for(D6W = 3; D6W != 0; D6W--)
   {
      for(D7w = 4; D7w != 0; D7w--)
      {
         ...
         for(D5W = sw(D6W - 1); D5W != 0; D5W--)
         {
            ...
            if(testdelay)
               VBLDelay(_10_, MS2VBL(testdelay));
            //
         }
         //
         //
   ...
   CreateCloud(RecordName(RNFireball), 0xff, w_4, w_6, w_4, w_6, 255, NULL);
   ProcessTimersViewportAndSound(_12_); // TAG01fed6(_12_)
   // pauseAwhile(800);
   VBLDelay(_24_, MS2VBL(800));
   CreateCloud(RecordName(RNDispellMissile), 0xff, w_4, w_6, w_4, w_6, 255, NULL);
   ...
   if(testdelay)
      VBLDelay(_26_, MS2VBL(testdelay));
   d.Word1868 = 1;
   ProcessTimersViewportAndSound(_14_); // TAG01fed6(_14_);
   if(testdelay)
      VBLDelay(_27_, MS2VBL(testdelay));
