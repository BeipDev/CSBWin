...
void PLAYFILE::Open()
{
   ...
   {
      char msg[200];
      if((strcmp(szCSBVersion, "CSB for Windows Version 9.03") == 0) && (strcmp(versionLine + 1, "CSB for Windows Version 9.0") == 0))
      {
...
bool PLAYFILE::Play(MouseQueueEnt *ent)
{
   ...
   { // Must read another record from file.
      ...
      if(m_forceClose || GETS(line, 80, m_file) == NULL)
      {
         ...
         if(!m_forceClose && (d.Time <= m_oldTime))
            return false;
         MouseQueueEnt tent;
         tent.num = 0x5555;
         tent.x = 0;
   ...
   ASSERT((m_time >= d.Time) || (m_time == 0) || (m_num == 0xffff) || (m_num == 0x5555) || (m_num == 0x6668), "playback");
   if(m_time > d.Time)
      return false;
   if((m_time != 0) && (m_ran != d.RandomNumber) && (m_ran != numRandomCalls - m_oldCallCount) && (m_num != 0x2222) // Cycle random number generator
...
void RECORDFILE::Signature(ui32 sig, ui32 type)
{
   if(m_fileNum >= 0)
   {
      fprintf(GETFILE(m_fileNum), "%08x %04x %04x %04x %08x\n", d.Time, (sig >> 16) & 0xffff, sig & 0xffff, type, d.RandomNumber);
   }
   STRandom();
}
...
{
   if((ent->num == 0x64) && (ent->x == 0x114) && (ent->y == 0x2f))
      return; // No operation
   if(ent->num < 0x2000)
   {
      totalMoveCount++;
   }
   if((m_fileNum < 0) && !m_isQueueingLines)
      return;
   if(m_fileNum >= 0)
   {
      if(!PlayFile.IsOpen() && d.ClockRunning)
      {
         ...
         if(((d.RandomNumber & 0x351a8) == 0) || (!m_graphicSignature) && (graphicSignature1 != 0))
         {
            Signature(graphicSignature1, 0x6666);
            m_graphicSignature = true;
         }
         if(((d.RandomNumber & 0x351a4) == 0) || (!m_CSBgraphicSignature) && (CSBgraphicSignature1 != 0))
         {
            Signature(CSBgraphicSignature1, 0x666a);
            m_CSBgraphicSignature = true;
         }
         if(((d.RandomNumber & 0x8a153) == 0) || (!m_dungeonSignature) && (dungeonSignature1 != 0))
         {
            Signature(dungeonSignature1, 0x6667);
            m_dungeonSignature = true;
         }
         if(((d.RandomNumber & 0x8a163) == 0) || (!m_versionSignature) && (versionSignature != 0))
         {
