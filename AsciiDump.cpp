...
public:
   ...
   void Allocate();
   void Print(FILE *f);
   void MarkLocation(RecordName obj, i32 level, i32 x, i32 y, i32 container);
};
...
void MISCTABLE::Allocate()
{
   ...
   for(i = 0; i < db.NumEntry(dbMISC); i++)
   {
      ...
      if(pDB10->link() == RNnul)
         miscEntry[i].level = 0xfe;
   }
}

void MISCTABLE::MarkLocation(RecordName obj, i32 level, i32 x, i32 y, i32 container)
{
   i32 index;
   if(obj.dbType() != dbMISC)
      return;
   index = obj.idx();
...
void MISCTABLE::Print(FILE *f)
{
   ...
   for(i = 0; i < db.NumEntry(dbMISC); i++)
   {
      ...
      else if((miscEntry[i].level >= 101) && (miscEntry[i].level <= 104))
      {
...
void MarkQueuedMonsterPossessionLocations()
{
   ...
   // for (i=1; i<gameTimers.NumTimer(); i++) xxTIMERxx Change to FindNextTimer
   {
      ...
      if((pTimer->Function() == TT_60) || (pTimer->Function() == TT_61))
      {
         // pTimer = d.pTimer(d.TimerQue(i));
         obj = pTimer->timerObj8();
         if(obj.dbType() != dbMONSTER)
            continue;
...
void TARGETS::Add(i32 t, i32 sl, i32 sx, i32 sy, i32 tl, i32 tx, i32 ty)
{
   if(m_allocated <= m_numTarget)
   {
      ...
      m_targets = (i32 *)UI_realloc(m_targets, m_allocated * sizeof(i32), MALLOC059);
   }
...
void PrintLink(FILE *f, RecordName link)
{
   if(link == RNeof)
   {
      fprintf(f, "LK=eof       ");
   ...
   {
      fprintf(f, "LK=%d,%02d,%04d ", link.pos(), link.dbType(), link.idx());
   }
}
...
{
   i32 num, react;
   fprintf(f, "     %03d %s ", pDB3->value(), MonsterName((MONSTERTYPE)pDB3->value()));
...
void DumpDB0(FILE *f, RecordName object, CELLFLAG cf)
{
   DB0 *pDB0;
...
void DumpDB1(FILE *f, RecordName object, CELLFLAG /*cf*/)
{
   DB1 *pDB1;
...
void printWallGraphic(FILE *f, DB3 *pDB3, RecordName object,
                      i32 level, i32 x, i32 y)
{
   ...
   pWallDecorationTOC = d.LevelCellFlags[d.width - 1] + d.height + pLevelDesc->numMonsterType();
   switch(pos)
   {
      case 0: y--; break;
      case 1: x++; break;
   ...
   switch(wallGraphicNum)
   {
      ...
      case 54:
         ...
         break;
      default:
         fprintf(f, "Unknown Wall Graphic ordinal = %d num= %d.", wallGraphicOrdinal, wallGraphicNum);
   }
}
...
void DumpDB3(FILE *f, RecordName object,
             i32 level, i32 x, i32 y, CELLFLAG cf)
{
   ...
   fprintf(f, "    ACTUATOR");
   fprintf(f, "[%04x %04x %04x %04x] ", (ui16)pDB3->link().ConvertToInteger(), (ui16)pDB3->word2, (ui16)pDB3->word4, (ui16)pDB3->word6);
   if(pDB3->localActionOnly())
   { // Some sort of local action
      fprintf(f, "typ=%02d val=%03d\n", pDB3->actuatorType(), pDB3->value());
   ...
   else
   {
      ...
      if(rt == roomSTONE)
      {
         ...
         switch(pDB3->actuatorType())
         {
            ...
            case 3:
               printWallGraphic(f, pDB3, object, level, x, y);
               fprintf(f, " Operated by %s", d.ObjectNames[pDB3->value()]);
            ...
            case 6:
            case 48:
               fprintf(f, "A counter.  Current value is %d.  ", pDB3->value());
...
void DumpDB4(FILE *f, RecordName object, i32 level, i32 x, i32 y)
{
   DB4 *pDB4;
   MONSTERTYPE mt;
   char *obName;
...
void DumpDB5(FILE *f, RecordName object, i32 /*level*/)
{
   DB5 *pDB5;
   OBJ_NAME_INDEX objNI;
   ...
   fprintf(f, "    object %04d ", object.idx());
   PrintLink(f, pDB5->link());
   fprintf(f, "0x%04x ", (ui16)pDB5->word2);
...
void DumpDB6(FILE *f, RecordName object, i32 /*level*/)
{
   DB6 *pDB6;
   OBJ_NAME_INDEX ot;
   char *obName;
   ...
   fprintf(f, "0x%04x ", (ui16)pDB6->word2);
   ot = object.NameIndex();
...
void DumpDB7(FILE *f, RecordName object)
{
   DB7 *pDB7;
...
void DumpDB8(FILE *f, RecordName object, i32 /*level*/)
{
   DB8 *pDB8;
   OBJ_NAME_INDEX objNI;
   char *obName;
   ...
   fprintf(f, "0x%04x ", (ui16)pDB8->word2);
   objNI = object.NameIndex();
   obName = d.ObjectNames[objNI];
...
void DumpDB9(FILE *f, RecordName object, i32 level, i32 x, i32 y)
{
   DB9 *pDB9;
   OBJ_NAME_INDEX objNI;
   RecordName containedObject;
   ...
   fprintf(f, "0x%04x 0x%04x 0x%02x 0x%02x", (ui16)pDB9->contents().ConvertToInteger(), (ui16)pDB9->word4(), (ui16)pDB9->uByte6(0), (ui16)pDB9->uByte6(1));
   containedObject = pDB9->contents();
   while(containedObject != RNeof)
   {
      if(containedObject.dbType() == dbMISC)
...
void DumpDB10(FILE *f, RecordName object, i32 /*level*/)
{
   DB10 *pDB10;
   OBJ_NAME_INDEX objNI;
   char *obName;
   ...
   fprintf(f, "0x%04x ", (ui16)pDB10->word2);
   objNI = object.NameIndex();
   obName = d.ObjectNames[objNI];
   fputs(obName, f);
   fprintf(f, "\n");
...
void DumpDB(FILE *f, RecordName object, CELLFLAG cf, i32 level, i32 x, i32 y)
{
   if(f == NULL)
      return;
...
void GetTargetDB0(RecordName, CELLFLAG, i32, i32, i32)
{
}

void GetTargetDB1(RecordName object, CELLFLAG, i32 level, i32 x, i32 y)
{ // Teleporter
   DB1 *pDB1;
   i32 tl, tx, ty;
   pDB1 = GetRecordAddressDB1(object);
...
void GetTargetDB3(RecordName object, CELLFLAG, i32 level, i32 x, i32 y)
{
   DB3 *pDB3;
   i32 tx, ty;
   pDB3 = GetRecordAddressDB3(object);
...
void GetTargetDB4(RecordName, CELLFLAG, i32, i32, i32)
{
}

void GetTargetDB5(RecordName, CELLFLAG, i32, i32, i32)
{
}

void GetTargetDB6(RecordName, CELLFLAG, i32, i32, i32)
{
}

void GetTargetDB9(RecordName, CELLFLAG, i32, i32, i32)
{
}

void GetTargetDB10(RecordName, CELLFLAG, i32, i32, i32)
{
}

void getTargetDB(RecordName object, CELLFLAG cf, i32 level, i32 x, i32 y)
{
   switch(object.dbType())
   {
      case dbDOOR:
   ...
   for(x = 0; x < d.width; x++)
      for(y = 0; y < d.height; y++)
      {
         ...
         while(object != RNeof)
         {
            fprintf(f, "  OBJ ");
            printAbsLocation(f, level, x, y);
            fprintf(f, "%02d obj=%1d,%02d,%04d\n", object.pos(), object.pos(), object.dbType(), object.idx());
...
{
   for(i32 i = 0; i < 27; i++)
      DumpMonster(f, i, d.MonsterDescriptor[i]);
}
...
{
   fprintf(f, "Object Index %3d = Scroll   Type %3d  Object Type = %3d = %s\n", indx, n, d.ObjDesc[indx].objectType(), d.ObjectNames[d.ObjDesc[indx].objectType()]);
}
...
void DumpClothing(FILE *f)
{
   ...
   for(i = 0; i < 58; i++)
   {
      fprintf(f, "%3d      %3d      0x%02x   0x%02x\n", i, d.ClothingDesc[i].Weight(), d.ClothingDesc[i].uByte1(), d.ClothingDesc[i].uByte2());
   }
}
...
void DumpWeapons(FILE *f)
{
   ...
   for(i = 0; i < 46; i++)
   {
      fprintf(f, " %3d     %3d   0x%02x  0x%02x  0x%02x  0x%04x\n", i, d.weapons[i].weight, d.weapons[i].uByte1, d.weapons[i].uByte2, d.weapons[i].uByte3, d.weapons[i].word4);
   }
}
...
void DumpObjects(FILE *f)
{
   ...
   for(i = 23; i <= 68; i++)
      DumpWeapon(f, i, i - 23);
   for(i = 69; i <= 126; i++)
      DumpClothing(f, i, i - 69);
   for(i = 127; i <= 179; i++)
      DumpMiscObject(f, i, i - 127);
}
...
void PopTimer(ui16 *que, i32 i, i32 n)
{
   ...
   if(gameTimers.pTimer(que[j]) < gameTimers.pTimer(que[k]))
      k = j;
   que[i] = que[k];
   PopTimer(que, k, n);
}
...
bool Ignore(i32 level, i32 x, i32 y, i32 what, i32 mode)
{
   ...
   {
      if(ignoreLocations[i] != level)
         continue;
      if(ignoreLocations[i + 1] != x)
         continue;
      if(ignoreLocations[i + 2] != y)
         continue;
      if((ignoreLocations[i + 3] & what) == 0)
         return true;
   }
   return false;
}
                    ...
                    i32 mode)
{
   ...
   if(objType == dbDOOR)
      return 0;
   if(objType == dbTELEPORTER)
      return 0;
   if(objType == dbTEXT)
      return 0;
   if(objType == dbACTUATOR)
      return 0;
   if(objType == dbMISSILE)
      return 0;
   if(objType == dbCLOUD)
      return 0;
   if(objType == dbMONSTER)
   {
      ...
      if(pass != 2)
         return 0;
      if(Ignore(level, x, y, MONSTER, mode))
         return 0;
      pDB4 = GetRecordAddressDB4(object);
      if(pDB4->monsterType() == mon_LordChaos)
         return 0;
      mt = pDB4->monsterType();
   ...
   if(objType == dbWEAPON)
   {
      ...
      if(pass != 1)
         return 0;
      if(Ignore(level, x, y, ANY, mode))
         return 0;
      pDB5 = GetRecordAddressDB5(object);
      wpnType = pDB5->weaponType();
      if(!pDB5->important())
         return 0;
      if(wpnType == weapon_Rock)
         return 0;
      if(wpnType == weapon_StoneClub)
         return 0;
      objNI = object.NameIndex();
   ...
   if(objType == dbMISC)
   {
      ...
      if(!pDB10->important())
         return 0;
      if(miscType == misc_Boulder)
         return 0;
      if(miscType == misc_Cheese)
         return 0;
      if(miscType == misc_Bread)
         return 0;
      if(miscType == misc_DragonSteak)
         return 0;
      if(miscType == misc_Shank)
         return 0;
...
bool listRemainingDBObject(char *line, RecordName object)
{
   i32 objType;
   objType = object.dbType();
   if(objType == dbDOOR)
...
{ // Mode 0 = CSB Challenge mode
  // Mode 1 = Print all remaining items
   ...
   ROOMTYPE rt;
   i32 x, y, pass, n;
   RecordName object, possession, containedObject;
   DB10 *pDB10;
   DB9 *pDB9;
   MISCTYPE miscType;
   LISTING listing;
   ...
   if(mode == 0)
   {
      if((d.partyLevel != 0) || (d.partyX != 20 - d.pLevelDescriptors[0].offsetX) || (d.partyY != 38 - d.pLevelDescriptors[0].offsetY))
      {
         ...
         for(x = 0; x < d.width; x++)
         {
            for(y = 0; y < d.height; y++)
            {
               ...
               while(object != RNeof)
               {
                  ...
                  if(n > 0)
                  {
                     printLocation(line, level, x + d.pLevelDescriptors[level].offsetX, y + d.pLevelDescriptors[level].offsetY);
