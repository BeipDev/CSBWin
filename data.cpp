...
{ // Translate physical pixel units on actual screen.
  //  to pixel units on Atari screen
   ...
   for(i = 0; i < 4; i++)
   {
      ...
      if(a < videoSegX[i])
         continue;
      if(a >= videoSegX[i] + size * (videoSegWidth[i]))
         continue;
      if(b < videoSegY[i])
         continue;
      if(b >= videoSegY[i] + size * (videoSegHeight[i]))
         continue;
...
RecordName::RecordName(ui32, ui32, ui32)
{
   NotImplemented(0x7cd0);
}

RecordName::RecordName(DBTYPE dbType, ui32 dbIndex)
{
   i32 emptyIndexEntry;
   i32 i, newDML;
   DBCOMMON *pDBC;
...
bool RecordName::checkIndirectIndex() const
{
   if((indirectIndex < dataMapLength) && (indirectIndex > 0))
      return true;
...
bool RecordName::checkIndirectIndex(ui32 i) const
{
   if((i < (unsigned)dataMapLength) && (i > 0))
      return true;
   return false;
}

void RecordName::CreateSpell(i32 n)
{
   ASSERT(n < 128, "spell");
   indirectIndex = (ui16)(0xff80 + n);
}

ui8 RecordName::GetSpellType()
{
...
//   TAG009308
OBJ_DESC_INDEX RecordName::DescIndex() const
{
   // UNKNOWN *unkA3;
   i32 DBType;
...
OBJ_NAME_INDEX RecordName::NameIndex() const
{
   ...
   if((objNID7 == objNI_Waterskin) || (objNID7 == objNI_Illumulet_a) || (objNID7 == objNI_JewelSymal_a))
   {
   ...
   if((objNID7 == objNI_Storm_a) || (objNID7 == objNI_Flamitt_a) || (objNID7 == objNI_StormRing_a) || (objNID7 == objNI_RABlade_a) || (objNID7 == objNI_EyeOfTime_a) || (objNID7 == objNI_StaffOfClaws_a))
   {
      D0W = (i16)dbA3->CastToDB5()->charges();
      if(D0W)
      {
...
bool RecordName::IsAKey() const
{
   OBJ_NAME_INDEX objNI;
   if(dbType() != dbMISC)
      return false;
   objNI = NameIndex();
   if(objNI < objNI_FirstKey)
      return false;
   if(objNI > objNI_LastKey)
      return false;
   return true;
}

bool RecordName::Levitating()
{
   DBTYPE dbType;
   DB4 *pDB_4;
   dbType = this->dbType();
...
const RecordName DBCOMMON::link()
{
   return m_link;
}

RecordName *DBCOMMON::pLink()
{
   return &m_link;
}

void DBCOMMON::link(RecordName rn)
{
   m_link = rn;
}

DBCOMMON *DATABASES::GetCommonAddress(RecordName object)
{
   i32 dbNum, index;
   dbNum = object.dbNum();
   index = object.idx();
   ASSERT(index < m_numEnt[dbNum], "numEnt");
   return (DBCOMMON *)(((char *)m_Address[dbNum]) + index * dbEntrySizes[dbNum]);
}
...
DBCOMMON *DATABASES::GetCommonAddress(DBTYPE dbType, i32 i)
{
   ...
   index = i;
   ASSERT(index < m_numEnt[dbNum], "numEnt");
   return (DBCOMMON *)(((char *)m_Address[dbNum]) + index * dbEntrySizes[dbType]);
}
...
void DATABASES::Allocate(i32 dbNum, i32 numEnt)
{
   ...
   if(numEnt > 0)
   {
      ...
      for(i32 i = 0; i < m_numEnt[dbNum]; i++)
      {
         ...
         *(RecordName *)p = RNnul; // Mark entry as empty
...
{ // Returns index of newly available entry or -1
   ...
   if(AutoEnlarge)
      result = MESSAGE_IDYES;
   else
      result = UI_MessageBox(msg, NULL, MESSAGE_YESNO);
...
DBTYPE DATABASES::GetDBType(DBCOMMON *pDB)
{
   ...
   {
      if((pDB >= m_Address[dbNum]) && (pDB < m_Address[dbNum] + dbEntrySizes[dbNum] / sizeof(*m_Address[0]) * m_numEnt[dbNum]))
         return (DBTYPE)dbNum;
   }
   return (DBTYPE)dbNum;
}
...
RecordName DATABASES::AllocateSegment(i32 /*dbNum*/, DBTYPE /*dbType*/)
{
   ...
   return RecordName(0, dbNum, 1);
   */
   return RNnul;
}

// TAG00a0b0
RecordName DATABASES::FindEmptyDBEntry(DBTYPE dbType, bool important)
{ //(RecordName)
...
void EXPOOL::AddChampionBonesRecord(const RecordName &bonesObject, ui32 fingerprint)
{
   ui32 record[1];
   ui32 key;
...
bool EXPOOL::CheckChampionBonesRecord(const RecordName &bonesObject)
{
   ui32 *pRecord;
   ui32 key;
   i32 size;
...
bool EXPOOL::CopyChampionBonesRecord(const RecordName &bonesObject, ui32 *pResult)
{
   ui32 *pRecord;
   ui32 key;
   i32 size;
...
bool EXPOOL::GetChampionBonesRecord(const RecordName &bonesObject, ui32 *pResult)
{
   ui32 key;
   i32 size;
   key = (EDT_ChampionBones << 24) | (bonesObject.ConvertToInteger() & 0xffff);
...
{ // result will be 8 bytes (6-bytes MD5 + 2 zeroes)
   ...
   fseek(f, 0, SEEK_SET);
   MD5Final(result);
   *signature1 = result[0] | (result[1] << 8) | (result[2] << 16) | (result[3] << 24);
   *signature2 = result[4] | (result[5] << 8) | (result[6] << 16) | (result[7] << 24);
   if(*signature1 == 0)
      *signature1 = 1; // one in 4 billion
   //  return sum == 0 ? 1 : sum;
}
...
void Cleanup(bool programTermination)
{
   ...
   if(programTermination)
   {
      ...
      g_root.clear();
      FreeIfNonNULL((void **)&globalVariables);
...
void SKIN_CACHE::SetSkin(i32 level, i32 x, i32 y, i8 skinNum)
{
   ...
   if(index < m_size[column])
   {
      if(skinNum == m_columns[column][index])
         return;
   }
   else
   {
   ...
   // works return;
   if(recSize < 64)
      memset(m_columns[column] + recSize, 0, 64 - recSize);
   // works return;
   m_columns[column][index] = skinNum;
   // works return;
   for(i = 63; i >= 0; i--)
   {
      if(m_columns[column][i] != 0)
         break;
   }
   if(i < 0)
      return;
