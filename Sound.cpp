...
{ // volume is a divisor
   ...
   pN = (char *)pGraphic;
   if(f)
      fprintf(f, "Starting pN = %p   Number of samples=%08x\n", (void *)pN, n);
   even = true;
   while(n > 0)
   {
      if(even)
      ...
      else
      {
         ...
         sample = *(pW - 1);
         if(f)
            fprintf(f, "Sample=%d, repeat=%d,n=%d\n", sample, repeat, n);
         for(i = 0; i < repeat; i++)
         {
            if(f)
            {
...
char *SOUNDER::Resample(char *wave)
{
   ...
   if(pSndHead->nSamplesPerSecond != 6000)
      return wave;
...
void PlayCustomSound(i32 soundNum, i32 volume, i32 /*flags*/)
{
   ...
   {
      if(!currentSound.ReadSound(soundNum))
         return;
      if(gameVolume == VOLUME_OFF)
         return;
      if(NoSound)
