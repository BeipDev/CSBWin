...
void DrawItem(i32 squareNumber, OBJ_NAME_INDEX objectNameIndex)
{
   ...
   if(objectNameIndex == objNI_NotAnObject)
      return;
...
// *********************************************************
// TAG009d9e
RecordName MakeMoreRoomInDB(DBTYPE dbType)
{ //(RecordName)
   ...
   if(index != -1)
   {
      return RecordName(dbType, index);
   }
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(dbType == dbCLOUD)
   {
   ...
   for(;;)
   {
      ...
      //
      if((D4W == d.partyLevel) || (d.dungeonDatIndex->NumLevel() <= 1)) // # levels
      {
         d.LevelCleared[dbType] = D4UB;
         return (RNnul);
      }
...
{ // attackerPos is the position within the room
   ...
   attackFromDirection =
       GetPrimaryAndSecondaryDirection(
           victimX, victimY, attackerX, attackerY);
...
bool FingerprintExists(ui32 fingerprint)
{
   ...
   if(character.GetFromWings((ui16)fingerprint, true))
      return true;
   return false;
}
...
{ //()
   ...
   for(illegalHero = true; illegalHero; A2 = (aReg)legalHero)
   {
      ...
      if((unsigned)newStamina <= MAXSTAMINA)
         pcA3->MaxStamina(newStamina);
      else
      {
      ...
      if(reincarnateAttributePenalty > 16)
         reincarnateAttributePenalty = 2;
      reincarnateStatPenalty = GetOneValue(&A2);
      if(reincarnateStatPenalty > 16)
         reincarnateStatPenalty = 8;
   ...
   while(firstObj != RNeof)
   {
      ...
      if(D4W > dbACTUATOR)
      {
         if(firstObj.pos() == LOCAL_12)
         {
            ...
            switch(D4W)
            {
               case dbCLOTHING:
                  ...
                  // Check Neck
                  if(((LOCAL_14 & d.CarryLocation[10]) != 0) && (pcA3->Possession(10) == RNnul))
                  {
                     LOCAL_6 = 10;
                  }
                  else
         ...
         switchbreak:
            if(pcA3->Possession(LOCAL_6) != RNnul)
               goto case9;
            if(LOCAL_6 < 30)
            {
...
void DisplayBackpackItem(i32 chIdx, i32 itemNum)
{
   ...
   else
   {
      ...
      if((D5W) && (D7W == 1) && ((objNID4 == objNI_Chest) || (objNID4 == objNI_OpenScroll)))
      {
         objNID4 = (OBJ_NAME_INDEX)(objNID4 + 1);
         ;
      }
...
{                                  //()
   ...
   if((charFlags & CHARFLAG_graphicFlags) == 0)
      return;
   ...
   if((charFlags & CHARFLAG_portraitChanged) != 0)
   {
      ...
      if(pcA3->HP() != 0)
      {
         ...
         {
            charFlags |= CHARFLAG_weaponAttack | CHARFLAG_ouches | CHARFLAG_statsChanged | CHARFLAG_cursor;
         }
      }
      else
      {
   ...
   if(pcA3->HP() != 0)
   {
      ...
      if((charFlags & CHARFLAG_statsChanged) != 0)
      {
         ...
         if(inventoryOpen)
         {
            ...
            if((pcA3->food < 0) || (pcA3->water < 0) || (pcA3->poisonCount != 0))
            {
               LOCAL_8 = 34;
            }
            else
    ...
    RecordName item)
{
   ...
   for(;;) // Fake to avoid 'goto's.  I like 'break's better.
   {
      ...
      if((DBType == dbWEAPON) && (possessionIndex >= 0) && (possessionIndex <= 12))
      ...
      if((DBType == dbCLOTHING) && (possessionIndex >= 0) && (possessionIndex <= 12))
      { // Weapon or clothing.  Person, purse, or quiver.
         // Handled together in the original.  I wroke them apart.
      ...
      if(possessionIndex == 1) // Weapon hand
      {
         ...
         else
         {
            ...
            if((objNI >= objNI_StaffOfClaws_a) && (objNI <= objNI_StaffOfClaws_c))
            {
               D4W = 4;
            }
            else
...
void SetCursed(RecordName obj, bool curse)
{
   ...
   if(pCH != NULL)
      AdjustStatsForItemCarried(pCH, loc, -1, obj);
   switch(obj.dbType())
   {
      case dbWEAPON:
...
// *********************************************************
// TAG015a66
RecordName RemoveCharacterPossession(i32 chIdx, i32 possessionIndex)
{ //(RecordName)
   dReg D0;
   RecordName objD5, objRemoved;
   ...
   if(possessionIndex >= 30)
   {
      ...
      d.rnChestContents[possessionIndex - 30] = RecordName(RNnul);
   }
   else
   {
      ...
      pcA3->SetPossession(possessionIndex, RecordName(RNnul), true);
   }
   if(objRemoved == RNnul)
   {
      return (RNnul);
   ...
   if(possessionIndex == 10)
   {
      if((objNID5 >= objNI_Illumulet_a) && (objNID5 <= objNI_Illumulet_b))
      {
      ...
      {
         if((objNID5 >= objNI_JewelSymal_a) && (objNID5 <= objNI_JewelSymal_b))
         {
            dbA2->CastToDB10()->value(0); // Of no use anymore
         }
      }
   ...
   if(possessionIndex < 2)
   {
      if(possessionIndex == 1)
      {
         ...
         if((objNID5 >= objNI_OpenScroll) && (objNID5 <= objNI_Scroll))
         {
            dbA2->CastToDB7()->open(false);
         }
      }
      if((objNID5 >= objNI_Torch_a) && (objNID5 <= objNI_Torch_d))
      {
...
{ //()
   ...
   { // In a hand
      ...
      { // In fighting hand
         ...
         if((objNID4 >= objNI_OpenScroll) && (objNID4 <= objNI_Scroll))
         {
      ...
      else
      {
         if(bool_2 && (place == 1))
         {
            if((objNID4 == objNI_Chest) || ((objNID4 >= objNI_OpenScroll) && (objNID4 <= objNI_Scroll)))
            {
               pcA3->charFlags |= CHARFLAG_chestOrScroll;
            }
         }
   ...
   { // Not in a hand.
      if(place == 10)
      { // Necklace
         if((objNID4 >= objNI_Illumulet_a) && (objNID4 <= objNI_Illumulet_b))
         {
         ...
         {
            if((objNID4 >= objNI_JewelSymal_a) && (objNID4 <= objNI_JewelSymal_b))
            {
               dbA2->CastToDB10()->value(1);
               // SETBITS14_15(dbA2->word(2), 1);
               objNID4 = (OBJ_NAME_INDEX)(objNID4 + 1);
...
bool IsCharacterLucky(CHARDESC *pChar, i32 luckNeeded, const char *traceID)
{
   ...
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   // D7W = sw(P2);
   if(STRandomBool() && (STRandom(100) > luckNeeded))
   {
      if(traceID != NULL)
      {
   ...
   if(currentLuck == 0)
      ...
      D0W = (i16)STRandom(attrA3[Luck].Current());
   // SET(D0W, D0W > D7W);
...
{ // Essentially:  0.5 * Dexterity*(1-load/Maxload/2)
   ...
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   quickness = pChar->Attributes[Dexterity].Current() + (ranResult = STRandom(8));
   if(traceID != NULL)
   {
...
i32 DetermineThrowingDistance(i32 chIdx, i32 hand)
{
   ...
   if((objD4 != RNnul) && (objD4.dbType() == dbWEAPON))
   {
...
i16 TAG01680a(i32 chIdx, i32 possessionIndex)
{
   ...
   for(D7W = 0; D7W < 2; D7W++)
   {
      ...
      if((objD4 == RNnul) || (objD4.dbType() != dbCLOTHING))
         continue;
   ...
   D7W = sw(D7W + pcA3->word64 + pcA3->shieldStrength + d.PartyShield + D6W);
   if(TimerTraceActive)
   {
      fprintf(GETFILE(TraceFile),
              "  Tag01680a D7W = %d\n",
...
{ //()
   ...
   { // Find a living party member.
      if((d.PotentialCharacterOrdinal != D5W + 1) && (pcA3->HP() != 0))
         break;
   }
...
i32 DamageCharacter(i32 chIdx, i32 damage, i16 mask, i16 P4)
{
   ...
   // See if there is a character death filter
   {
      ...
      // Let the DSA process the parameters
      {
         ...
         DB3 *pActuator;
         NEWDSAPARAMETERS dsaParameters;
         RecordName damageCharDSA = RNeof;
...
void CHARDESC::SetPossession(int index, RecordName newObject, bool processFilter)
{
...
// *********************************************************
//   TAG017ce4
void ShowChestContents(RecordName objChest, DB9 *pChest, i16 P3)
{
   dReg D7;
   RecordName chestContents;
   DBCOMMON *pChestContentsRecord;
   i32 i;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(d.objOpenChest == objChest)
      return;
...
public:
   ...
   {
      Clear();
      m_address = (char *)UI_malloc(strlen(a) + 1, MALLOC091);
      strcpy(m_address, a);
   }
...
// *********************************************************
//   TAG0183ea
void DescribeObject(RecordName object, i16 P2)
{ // Describe object???
   ...
   else
   {
      ...
      else
      {
         ...
         else
         {
            //
            //
            if((dbType == dbPOTION) && (objNID6 != objNI_WaterFlask) && (objNID6 != objNI_EmptyFlask) && (DetermineMastery(d.SelectedCharacterOrdinal - 1, 2) > 1))
            {
         ...
         switch(dbType)
         {
            ...
            case dbWEAPON:
               ...
               D1W = sw(DB5A0->broken());
               D4W |= D1W << 2; // Broken
               if((objNID6 >= objNI_Torch_a) && (objNID6 <= objNI_Torch_d))
               {
            ...
            case dbMISC: // Bones, for example.
               ...
               {
                  if((objNID6 >= objNI_Compass_N) && (objNID6 <= objNI_Compass_W))
                  {
                     descriptionMask = 0;
         ...
         if(phraseMask & 0x40)
            PrintItemDesc(descriptivePhrases[8], phraseColors[8]);
         if(phraseMask & 0x80)
            PrintItemDesc(descriptivePhrases[9], phraseColors[9]);
      }
   }
   TAG018238(P2);
}
...
{ //()
   ...
   static RecordName objD7;
   static CHARDESC *pcA0, *pcA2;
   static DBCOMMON *dbA3;
   static i16 w_40;
   static i16 w_30;
   static i16 w_20;
   static RecordName obj_w_20;
   ...
   if((D0W & 1) == 0)
   {
      if(objD7.dbType() == dbCHEST)
         RETURN; // Not consumable
      if(objD7.dbType() == dbSCROLL)
         RETURN;
   }
   else // if consumable
   {
   ...
   if(fp.performFeeding)
   {
      ...
      if(fp.shieldStrengthAdjust != 0)
      {
         ...
         {
            pcA2->shieldStrength = sw(pcA2->shieldStrength + fp.shieldStrengthAdjust);
         }
         newShieldLevel = pcA2->shieldStrength / 50;
         // if (oldShieldLevel != newShieldLevel)
         //{
      ...
      DrawCharacterState(chIdx);
      STShowCursor(HC6);
      if(itemToDeleteFromDatabase != NULL)
         DeleteDBEntry(itemToDeleteFromDatabase);
   }
   RETURN;
}
...
tag01b018:
   ...
   if((pcA2->charFlags & CHARFLAG_attackDisable) || (d.PotentialCharacterOrdinal) || (d.PartySleeping))
   {
...
{ //()
  // This function look for the object in hand and pops up
   ...
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   pcA3 = &d.CH16482[chIdx];
   if(((pcA3->charFlags & CHARFLAG_attackDisable) != 0) || (pcA3->HP() == 0))
      return;
...
{ //(i16)
   ...
   if((d.AttackingCharacterOrdinal == 0) || ((UI8)(d.PossibleAttackTypes[P1]) == 255))
   {
      RETURN_i16(0);
   }
   pChar = &d.CH16482[d.AttackingCharacterOrdinal - 1];
...
bool CHARDESC::GetFromWings(ui16 fingerprint, bool testOnly)
{
   ...
   if(pChar->fingerPrint != fingerprint)
      result = false;
   if(result)
   {
...
void RemoveCharacter(int chIdx)
{
   ...
   // pcA3 = &d.CH16482[0];
   {
      ...
      { // Find a living party member.
         if((chIdx != livingChar) && (d.CH16482[livingChar].HP() != 0))
            break;
      }
...
ui32 SwapCharacter(i32 index, i32 fingerPrint)
{
   ...
   if((index >= 0) && (index < d.NumCharacter))
   {
      ...
      if((d.NumCharacter < 2) && (fingerPrint < 0))
         return 2;
   }
   else
   {
   ...
   if(fingerPrint >= 0)
   {
      if((index == -1) && (d.NumCharacter == 4))
         return 3;
      if(!pChar->GetFromWings((ui16)(fingerPrint & 0xffff), true))
         return 4;
   }
