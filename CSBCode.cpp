...
i32 LE32(i32 lng)
{
   return ((lng >> 24) & 0xff) | ((lng >> 8) & 0xff00) | ((lng << 8) & 0xff0000) | ((lng << 24) & 0xff000000);
}
#endif
...
{
   if((pwDerivedGraphicSizes[graphicNum] != 0) && (pDerivedGraphicCacheIndex[graphicNum] != -1))
   {
      char msg[100];
      sprintf(msg, "Doubly defined graphic size\nGraphic=0x%x, oldSize=0x%x, newSize=0x%x", graphicNum, pwDerivedGraphicSizes[graphicNum], size);
      UI_MessageBox(msg, "Warning", MESSAGE_OK);
      if(size > pwDerivedGraphicSizes[graphicNum])
...
void DBank::SetDerivedGraphicCacheIndex(i32 graphicNum, i32 index)
{
   ...
   if((index == -1) && (graphicNum > 3))
      pwDerivedGraphicSizes[graphicNum] = 0;
}
...
tag000698: // We get here for each tick of the clock.
   ...
   d.vblCount = 0;
   if(d.newPartyLevel == -1)
      goto tag0006d0;
tag0006a4:
   DEBUGMAIN("tag0006a4")
   LoadPartyLevel(d.newPartyLevel); // Load graphics for level
   MoveObject(RecordName(RNnul), -1, 0, d.partyX, d.partyY, NULL, NULL);
   d.newPartyLevel = -1;
   DiscardAllInput();
tag0006d0:
   DEBUGMAIN("tag0006d0")
...
tag000730:
   ...
   if(d.GameIsLost == 0)
   {
      ...
      if((currentOverlay.m_p3 != 0) && (currentOverlay.m_p3 < d.Time))
         overlayActive = false;
   ...
   tag0007c8:
      ...
      */
      ...
      if(DiskMenuNeeded)
         DisplayDiskMenu(_9_);
      if(d.clockTick == 0)
         WaitForButtonFlash(); // If any active
      if(d.clockTick == 0)
         goto tag0007b4; // Usually 0
      if(d.ClockRunning == 0)
         goto tag0007b4;     // Usually 1
...
void vblInterrupt() // TAG000c3a
{
   ...
   if(d.Word23138 == 0)
   {
      ...
      D0W = d.Word23132;
      d.Word23132 = 0;
      if((d.Word23130 != 0) && (d.NewCursorShape == 0) && (D3W == D4W) && (D2W == D5W))
      {
         D0L = 1;
         d.Word23130 = 0;
      }
   ...
   {
      if(playerClock && (d.vblCount > 10) && !mouseQueueIsEmpty() && !IsPlayFileOpen())
      {
         d.clockTick = 1;
      }
   }
...
OBJ_NAME_INDEX GetBasicObjectType(RecordName object)
{
   // dReg D7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(object == RNnul)
...
{ //()
   ...
   for(D6W = 0; D6W < 4; D6W++)
   {
      ...
      //    wvbl(_1_);//NotImplemented(0x194e);//pumper();
      //  };
      if((d.TextScanlineScrollCount >= 0) || (d.PushTextUp != 0))
      {
         return; // We'll get it next time.  //pumper();
      }
      STHideCursor(HC20); // TAG002fd2
...
void SCROLLING_TEXT::ClockTick()
{
   ...
   {
      if(d.TextScanlineScrollCount >= 0)
         return; // wait for scrolling to cease
      if(d.PushTextUp != 0)
         return; // wait for line to scroll.
      if(m_futureLines.IsEmpty())
         return;
      pPiece = m_futureLines.Peek();
      if(pPiece->m_row == -1)
      { // A command to create new row of text at bottom.
         if((d.TextTimeout[0] != -1) && (m_futureLines.MaxPrintLinesCount() <= m_printLinesCount[0]))
            return;
         ScrollUp();
      ...
      if((row > 0) && (d.TextTimeout[row - 1] != -1) && (d.TextTimeout[row] <= d.TextTimeout[row - 1]))
      {
...
void QueueSound(i32 soundNum, i32 mapX, i32 mapY, i32 deltaTime)
{
   ...
   if(D7W >= (UI8)psA3->byte7)
      LOCAL_12 = -1;
   if(SoundFilter(soundNum, LOCAL_12, &soundLocr) == 0)
      return; // The Filter sill handle the sound.
   if(LOCAL_12 < 0)
      return;
   /*
   d.Word23116++;
...
tag0029ba:
   ...
   if(D5W == d.SelectedCharacterOrdinal)
      goto tag0029f8;
   if(D6W <= 6)
      goto tag0029f8;
tag0029e0:
...
{ // We also draw wall text.
   ...
   if(drawingText && (text == RNeof))
      return 0;
   // D6W = P2;
   if((graphicOrdinal == 0) && (championPortraitOrdinal == 0))
      return 0;
      ...
      if(relativePosition == 12) // Wall immediately in front of party
      {
         if(drawingText)
         {
            ...
            do
            {
               ...
               A2 = A3;
               while((UI8)(*(A2++)) < 128)
                  lineLength++; // Count characters in this line
               if(lineLength > 18)
                  lineLength = 18;
   ...
   { // relativePosition < 10 (F2 and farther)
      ...
      {
         derivedGraphicIndex = 4 + 4 * graphicIndex + d.WallDecorationDerivedGraphicOffset[relativePosition];
      ...
      {
         {
            ...
            //
            {
               ...
               if(width > pSrcSizeDescriptor->width)
                  width = pSrcSizeDescriptor->width; // width in bytes
               if(height > pSrcSizeDescriptor->height)
                  height = pSrcSizeDescriptor->height;
               // ShrinkBLT(LOCAL_4,                                 //Source
               ShrinkBLT(pBasicBitmap,                                                                                                                 // Source
                         extendedWallDec ? wallDecCache.GetDerivedGraphicAddress(derivedGraphicIndex) : GetDerivedGraphicAddress(derivedGraphicIndex), // Destination
   ...
   if(graphicIndex >= 0)
   {
      ...
      if((pWDP - &d.wallDecorationPosition[0][0] == 0x5e) && (pWDP->rect.x1 == 0x4a) && (pWDP->rect.x2 == 0x95) && (pWDP->rect.y1 == 0x19) && (pWDP->rect.y2 == 0x4b) && (srcByteWidth == 48))
      {
         srcByteWidth = 40;
      }
      ...
      //      &&(srcByteWidth == 48)
      //   )
      if((pWDP - &d.wallDecorationPosition[0][0] == 0x5f) && (pWDP->rect.x1 == 0x8b) && (pWDP->rect.x2 == 0xdf) && (pWDP->rect.y1 == 0x19) && (pWDP->rect.y2 == 0x4b) && (srcByteWidth == 48))
      {
         srcByteWidth = 40;
      }
   }
   ...
   else
   {
      ...
      if(graphicIndex < 0)
         isAlcove = 0; // Allow pExtWallRects to be free'd
   }
   if(pExtWallRects != NULL)
      UI_free(pExtWallRects);
   return isAlcove;
...
void DrawFloorDecorations(i32 graphicOrdinal, i32 relativeCell)
{
   ...
   if(!extFloorDec && (graphicOrdinal == 0))
      return;
   ...
   if((D4W == 0) || extFloorDec || ((graphicOrdinal = (I16)(graphicOrdinal & 0x7fff)) != 0)) // Seems silly
   {
      ...
      else
      {
         ...
         graphicNumber =
             d.LoadedFloorDecorations[graphicIndex][0] + d.RelativeCellGraphicIncrement[relativeCell];
         // D1W = P2;
         // A0 = d.Byte4034 + D1W;
         // D3W = (UI8)(*A0);
         // D3W = d.Byte4034[P2];
      ...
      if(extFloorDec)
      {
         ...
         useMirror = (((mirrorFlags >> (d.partyFacing + 9)) ^ (mirrorFlags >> relativeCell)) & 1) != 0;
      }
      else
      {
         useMirror =
             ...
             || (relativeCell == 5)                   // F2R1
             || (relativeCell == 2)                   // F3R1
             || (!extFloorDec && (graphicIndex == 15) // footprints
                 && (d.UseMirrorImages != 0) && ((relativeCell == 7) || (relativeCell == 4) || (relativeCell != 1)));
         //               F1L1                 F2L1                 F3L1
      }
      if(useMirror)
      {
...
bool IsAlcove(i32 wallDecorationIndex)
{
   dReg D0, D7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(wallDecorationIndex < 0)
      return 0;
   for(D7W = 0; D7W < 3; D7W++)
   {
      D0W = d.AlcoveGraphicIndices[D7W];
      if(wallDecorationIndex == D0W)
         return 1;
      //
   }
   return 0;
   ...
   else
   {
      ...
      { // Then figure out what to do about illegal x.
         if(mapX == -1)
         {
            ...
            if(rtD4 == roomOPEN)
               return 4; // Stone...Random decoration on east side
            if(rtD4 == roomPIT)
               return 4;
            return 0;
         }
         if(mapX == d.width)
         {
            ...
            if(rtD4 == roomOPEN)
               return 1; // Stone...Random deoration on west side
            if(rtD4 == roomPIT)
               return 1;
            return 0;
         }
      ...
      if(legalX)
      {
         ...
         if(mapY == -1)
         {
            ...
            if(rtD5 == roomOPEN)
               return 2; // Stone...Random decoration on south side
            if(rtD5 == roomPIT)
               return 2;
            return 0;
         }
         if(mapY == d.height)
         {
            ...
            if(rtD5 == roomOPEN)
               return 8; // Stone ... Random decoration on north side
            if(rtD5 == roomPIT)
               return 8;
            return 0;
         }
...
//
// *********************************************************
DBCOMMON *GetCommonAddress(RecordName recDesc)
{
...
//
// *********************************************************
RecordName FindEmptyDB2Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB3Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB4Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB5Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB6Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB8Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB10Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB14Entry(bool important)
{ //(RN)
   RecordName result;
...
//
// *********************************************************
RecordName FindEmptyDB15Entry(bool important)
{ //(RN)
   RecordName result;
...
// *********************************************************
// TAG009a02
RecordName GetDBRecordLink(RecordName recName)
{
   // recDesc(bits 10-13) = database number
   // recDesc(bits 0-9)   = record number within database
   DBCOMMON *dbA0;
...
//  TAG009a94
RecordName FindFirstObject(i32 x, i32 y)
{

   i16 LOCAL_2;
   LOCAL_2 = GetObjectListIndex(x, y);
   if(LOCAL_2 == -1)
      return RecordName(RNeof);
   return d.objectList[LOCAL_2];
}

// TAG009ac8
RecordName FindFirstDrawableObject(i32 x, i32 y)
{
   RecordName obj;
...
// *********************************************************
//   TAG009c4e
void RemoveObjectFromRoom(RecordName object, i32 mapX, i32 mapY, MMRECORD *pmmr)
{
   dReg D0, D1, D5, D6;
   DBCOMMON *dbA3 = NULL;
   RecordName objD4, objD7;
   RecordName *prnA2;
   RecordName P2;
   ui16 *puwA2;
   CELLFLAG *pdA0;
   ...
   if(mapX >= 0)
   {
      ...
      if((dbA3->link() == RNeof) && ((*prnA2).IsSameObjectAs(objD7)))
      {
         ...
         d.objectList[D5W] = RecordName(RNnul); // clear last entry
...
void RemoveObjectFromRoom(RecordName object, const LOCATIONREL &locr)
{
                             ...
                             bool bcd)
{
   int i;
   CHARDESC *pCh;
   if(characterIndex >= d.NumCharacter)
      return false;
...
bool SubstituteCharacterNames(pnt pText, i32 max, bool bcd)
{
   ...
   char cmd;
   if((bcd && ((len = bcdstrstr((char *)pText, "\033\020\033\200")) < 0)) || (!bcd && (strstr((char *)pText, ".Q.") == NULL) && (strstr((char *)pText, ".R.") == NULL) && (strstr((char *)pText, ".N.") == NULL)))
      return true;
   if(!bcd)
      ...
      return true;
   {
      ...
      while(result && (i < len))
      {
         ...
         {
            if((pText[i + 1] + offset == 'Q') && (pText[i + 3] + offset >= 'A') && (pText[i + 3] + offset <= 'K'))
            {
               i += 3; // Skip over the ".Q."
            ...
            else if((pText[i + 1] + offset == 'R') && (pText[i + 3] + offset >= 'A') && (pText[i + 3] + offset <= 'A' + 15) && (pText[i + 4] + offset >= 'A') && (pText[i + 4] + offset <= 'A' + 15) && (pText[i + 5] + offset >= 'A') && (pText[i + 5] + offset <= 'A' + 15) && (pText[i + 6] + offset >= 'A') && (pText[i + 6] + offset <= 'A' + 15))
            {
               i += 3; // Skip over the ".R."
               k = i + 4;
               for(index = 0; i < k; i++)
            ...
            else if((pText[i + 1] + offset == 'N') && (pText[i + 3] + offset >= 'A') && (pText[i + 3] + offset <= 'A' + 15) && (pText[i + 4] + offset >= 'A') && (pText[i + 4] + offset <= 'A' + 15))
            {
               i += 3; // Skip over the ".N."
               k = i + 2;
               for(index = 0; i < k; i++)
...
{ // Wall text P3 = 0;
   ...
   // if (D7W != 0)
   {
      if(!SubstituteCharacterNames((pnt)pDest, max, typeOfText == 0))
         return -1;
   }
   return 0;
}
               ...
               i32 P8)     // 24
{
   ...
   rslt->decorations[2] = RandomWallDecoration(
       ...
       (mapY + 1) * (((facing + 3) & 3) + 1),
       30);
   if((P8 != 0) || (mapX < 0) || (mapX >= d.width) || (mapY < 0) || (mapY >= d.height))
...
//   TAG00b062
void AttachItem16ToMonster(RecordName P1, i32 mapX, i32 mapY)
{
   dReg D0, D1, D6;
   aReg A0;
   MONSTERDESC *pmtDesc;
   ...
   while(pi16A2->word0 >= 0)
   {
      ...
      if(item16Index >= d.MaxITEM16)
      {
         ...
         if(d.MaxITEM16 < 1050)
         {
            ...
            d.Item16 = (ITEM16 *)UI_realloc(d.Item16, 16 * newMax, MALLOC060);
...
void DetachItem16(i32 P1)
{
   ...
   //
   //
   if((P1 > d.MaxITEM16) || (d.Item16[P1].word0 < 0))
      return;
   //
...
{ //(i16)
   ...
   D6W = srcPossession;
   D5W = dstPossession;
   if(pChar->Possession(D6W) == RNnul)
      return 0;
   if(pChar->Possession(D6W).dbType() != dbWEAPON)
      return 0;
...
{                    //()
   ...
   while(gameTimers.CheckForTimerEvent() != 0)
   {
      ...
      if((function >= TT_FIRST_MONSTER_AI) && (function <= TT_LAST_MONSTER_AI))
      {
         ProcessTimers29to41(
      ...
      else
      {
         ...
         switch(function)
         {
            ...
            case TT_77:
               ...
               break;
            case TT_78:
               d.FireShield = sw(d.FireShield - timeEnt.timerWord6());
               MarkAllPortraitsChanged();
               break;
...
//   TAG0156c4
void ObjectToCursor(RecordName object, i16 P2)
{ //()
...
i16 SearchFootprints(i32 mapX, i32 mapY)
{
   i32 index;
   i16 searchValue;
   searchValue = sw(((d.LoadedLevel & 63) << 10) | (mapY << 5) | (mapX & 31));
...
{ //()
   ...
   for(D7W = 0; D7W < d.NumCharacter; D7W++, pcA3++)
   {
      ...
      if(d.PotentialCharacterOrdinal == D7W + 1)
         continue;
      D5W = pcA3->HP();
      if(D5W == 0)
         continue;
...
{ //()
   ...
   if(chIdx == -1)
      return;
   if(chIdx + 1 == d.PotentialCharacterOrdinal)
      return;
   pch_14 = &d.CH16482[chIdx];
   DamageCharacter(chIdx, Larger(1, P2 / 64), 0, 0);
   pch_14->charFlags |= CHARFLAG_statsChanged;
   if((chIdx + 1 == d.SelectedCharacterOrdinal) && (d.DisplayResurrectChestOrScroll == 0))
   {
      ...
      for(D5W = 0; D5W < 4; D5W++, pCharacter++)
      {
         for(D4W = 2; (D4W--) != 0; pwA3++)
         {
            *pwA3 = 0;
            ...
            if((objNID0 >= objNI_Torch_a) && (objNID0 <= objNI_Torch_d))
            {
...
{ //()
   ...
   ASSERT(RememberToPutObjectInHand == -1, "objInHand");
   if(d.objectInHand == d.objOpenChest)
      RepackChest();
   TAG0189d4();
   MarkViewportUpdated(0);
...
{ //()
   ...
   if(chIdx != 4)
   {
      ...
      if(D0W == 0)
         return; // Don't show inventory of dead character
   }
   ...
   pcA3->charFlags |= CHARFLAG_viewportChanged | CHARFLAG_portraitChanged | CHARFLAG_chestOrScroll | CHARFLAG_possession | CHARFLAG_statsChanged | CHARFLAG_cursor;
...
i16 SearchButtonList(btn *ent, i32 x, i32 y, i32 buttons) // TAG019570
{
   ...
   else
   {
      ...
      {
         if((D7W < ent->xMax) && (D7W >= ent->xMin) && (D6W >= ent->yMin) && (D6W < ent->yMax))
         {
...
void OnMouseUnClick()
{
   ...
   {
      d.MouseQEnd++;
      if(d.MouseQEnd > MOUSEQLEN - 1)
         d.MouseQEnd -= MOUSEQLEN;
      // d.MouseQEnd = D5W;
...
{ //()
   ...
   MoveObject(RecordName(RNnul), d.partyX, d.partyY, -1, 0, NULL, NULL);
...
{ //()
   ...
   if(!gravityMove)
   {
      ...
      switch(direction)
      {
         ...
         case 2:
         case 3:
            absdir = (direction + d.partyFacing) & 3;
            axis = (absdir + 1) & 1;
            delta = ((absdir + 1) & 2) >> 1;
            break;
         // case 4:
   ...
   if(((partyMoveData.flags & PM_SETDELAY) != 0) && ((partyMoveData.flags & PM_ADDDELAY) == 0))
   {
   ...
   else
   {
      ...
      else
      {
         ...
         if((newLevel > d.dungeonDatIndex->NumLevel() - 1) || (newLevel < 0))
         {
            rtTo = roomSTONE;
         ...
         if((rtTo != roomSTONE) && FindFirstMonster(newLevel, newX, newY) != RNeof)
         {
            return result;
         }
      }
      if((rtTo == roomSTAIRS) || ((rtTo != roomSTONE) && (direction > 3)))
      {
         if(direction <= 3)
         {
      ...
      else
      {
         ...
         else
         {
            if(rtTo == roomDOOR)
            {
               ...
               D5W = ((doorCondition != 0) && (doorCondition != 1) && (doorCondition != 5));
               // D5W = D7W & 7;//Door type
               // bD0 = D5W!=0; //???
               // if (bD0)
               //{
         ...
         else
         {
            ...
            {
               MoveObject(RecordName(RNnul), -1, 0, newX, newY, NULL, NULL);
            }
            else
            {
               MoveObject(RecordName(RNnul), d.partyX, d.partyY, newX, newY, NULL, NULL);
            }
            time2move = 1;
...
{ //()
   ...
   if(chIdx == d.HandChar)
      return; // owner of cursor
   if((chIdx != -1) && (d.CH16482[chIdx].HP() == 0))
      return;
   if(d.HandChar != -1) // owner of cursor
   {
...
void TAG0196da(i32 key)
{
   ...
   if(pxA3 != NULL)
   {
      d.MouseInterlock = 1;
      if((key & 0xffff) == 0)
         key >>= 8; // Move scan code to lower word
      {};
...
RecordName actionNameDSA = RNnul;
...
{                                     //()
   ...
   if(checkExist || alwaysDate)
   {
      ...
      for(i = 0; i < 4; i++)
      {
         ...
         for(k = 1; k < 4; k++)
         {
            if(modifiedTime[k] < modifiedTime[j])
               j = k;
         }
         // j=smallest time
...
void ReadTablesFromGraphicsFile()
{
   ...
   ASSERT((d.GraphicDecompressedSizes[0x231] == 0x804) || (d.GraphicDecompressedSizes[0x231] == 0x7d4), "graphicSize231");
   ...
   for(i = 0; i < 27; i++)
      fixItem26(&d.MonsterDescriptor[i]);
   for(i = 0; i < 8; i++)
      d.FoodValue[i] = LE16(d.FoodValue[i]);
   for(i = 0; i < 180; i++)
   {
   ...
   for(i = 0; i < 4; i++)
      swapNwords((i16 *)&d.wRectPos1454[i], 4);
   ...
   for(i = 0; i < 6; i++)
      swapNwords((i16 *)&d.Palette552[i], 16);
...
void DisableTraceIfEnciphered()
{
   ...
   if((encipheredDataFile == NULL) && !simpleEncipher)
      return;
...
{ // return true if graphic already exists
   ...
   else
   {
      ...
      {
         if(d.pGraphicCachePointers[cacheIndex] == NULL)
            break;
      }
