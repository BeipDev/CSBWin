...
BOOL InitInstance(HINSTANCE hInstance, i32 nCmdShow)
{
   ...
   hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW, WindowX, WindowY, rc.right - rc.left, rc.bottom - rc.top, NULL, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND _hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   switch(message)
   {
      ...
      case WM_ACTIVATEAPP:
         if(wParam)
            MTRACE("WM_ACTIVATEAPP(true)\n");
         else
            MTRACE("WM_ACTIVATEAPP(false)\n");
         return DefWindowProc(hWnd, message, wParam, lParam);
      case WM_NCACTIVATE:
         ...
         switch(LOWORD(lParam))
         {
            ...
            case 1: // Misc menu
            {
               ...
               EnableMenuItem((HMENU)wParam,
                              ...
                              MF_BYCOMMAND | flag);
               flag = MF_GRAYED;
               if(ItemsRemainingOK && (encipheredDataFile == NULL) && !simpleEncipher)
                  flag = MF_ENABLED;
               EnableMenuItem((HMENU)wParam,
                              IDC_ItemsRemaining,
                              MF_BYCOMMAND | flag);
