...
void DebugCast(i32 i, i32 line, char *file)
{
   ...
   sprintf(msg, "A debug error has occurred.  It\n"
           ...
           line,
           file);
   if(UI_MessageBox(msg, NULL, MESSAGE_YESNO) == MESSAGE_IDNO)
      die(0);
}
...
void OnMouseSwitchAction(i32 buttons, i32 x, i32 y)
{
   ...
   if(d.OnMouseSwitchActionLock != 0)
      return;
...
void checkVBL()
{
   ...
   if((NoSpeedLimit == 0) || ((VBLCount % 200) == 0))
   {
      ...
      if((pointX >= 0) && (pointX < 320) && (pointY >= 0) && (pointY < 200))
      {
         if(!IsPlayFileOpen())
         {
...
#endif
   ASSERT(numState != 0, "numState");
   EC switch(stateStack[numState - 1].state)
   {
      case st_StartCSB:
...
void DebugLoad(void *p, i32 a, i32 b, RecordName obj)
{
   static FILE *f = NULL;
   static i32 additions[maxloaditems];
   static i32 timeadd[maxloaditems];
   static RecordName objadd[maxloaditems];
   ...
   if(b == -1)
      ...
      newLoad = oldLoad + a;
   fprintf(f, "%8d %2d %2d %5d  %4d --> %4d\n", d.Time, index, b, a, oldLoad, newLoad);
   if(b == 0)
      return;
   if(a == 0)
      return;
