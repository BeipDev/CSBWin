...
// *********************************************************
//  TAG0091e6
i32 GetObjectWeight(RecordName item)
{ // Note...recursive for chests.
   ...
   switch(item.dbType())
   {
      ...
      case dbCHEST: {
         ...
         RecordName objContents = pDB9->contents();
...
// *********************************************************
// TAG00a16e
RecordName ConstructPredefinedMissile(ui16 missileType)
{
   ...
   // D7W = P1;
   // dbTypeD5 = dbWEAPON;
   if((missileType >= 4) && (missileType <= 7))
      missileType = 4;
   switch(missileType)
   {
      case 54:
...
// *********************************************************
// TAG012d80
RecordName FindObjectOfTypeAtPosition(i32 mapX,
                                      i32 mapY,
                                      i16 pos,
                                      OBJ_NAME_INDEX objectNameIndex)
{
   ...
   while(objD5 != RNeof)
   {
      ...
      if(objNID0 == objNID6)
      {
         ...
         ASSERT(pos < 4, "Pos");
         if((pos == -1) || (objD5.pos() == pos))
            return objD5;
      }
      objD5 = GetDBRecordLink(objD5);
   }
   return RecordName(RNnul);
}
...
{ //(i16)
   dReg D0, D6;
   RecordName objD0, objD5;
   OBJ_NAME_INDEX objNID0;
   DBCOMMON *dbA3;
   DB3 *DB3A2;
   RecordName currentListObject;
   OBJ_NAME_INDEX objNI_38;
   DBCOMMON *pdb_28;
   RecordName firstListObject;
   RecordName previousListObject;
   RecordName obj_20;
   RecordName objectInHand;
   ...
   currentListObject = GetDBRecordLink(currentListObject))
   {
      ...
      if(D6W == dbACTUATOR) // Actuator
      {
         ...
         if(actuatorPos != touchedPosition)
            continue;
         ...
         switch(actuatorType)
         {
            ...
            case 4:
               ...
               {
                  if(previousListObject == currentListObject)
                     break;
            ...
            case 13: // A torch holder was type 13.
               if(d.EmptyHanded)
               {
                  objectInHand = FindObjectOfTypeAtPosition(
                      mapX, mapY, actuatorPos, OBJ_NAME_INDEX(actuatorValue));
                  if(objectInHand == RecordName(RNnul))
                     continue;
               ...
               else
               {
                  ...
                  AddObjectToRoom(
                      ...
                      mapY,
                      NULL);
                  objectInHand = RecordName(RNnul);
               }
               PerformLocalAction(2, mapX, mapY, actuatorPos);
               // D4W = 1;
               switchOpen = true;
               if((actuatorAction != actuatorAction_CONSTPRESSURE) || (d.EmptyHanded != 0))
               {
                  // D4W = 0;
                  switchOpen = false;
               }
            ...
            case 16:
               ...
               objNID0 = GetBasicObjectType(objectInHand);
               if((objNID0 != actuatorValue) || (obj_20 == RNnul))
                  continue;
               {
                  CURSORFILTER_PACKET cfp;
         ...
         if(!switchOpen)
         {
            ...
            if((!d.EmptyHanded) && ((actuatorType == 4) || (actuatorType == 11) || (actuatorType == 17)))
            {
               ...
               // dbA3->link(RNnul);
               {
                  ...
                  if(cfp.type != CURSORFILTER_Cancel)
                  {
                     ...
                     DeleteDBEntry(dbA3);
                     objectInHand = RecordName(RNempty);
                  }
               }
            }
            else
...
//*********************************************************
// TAG01576a
RecordName RemoveObjectFromHand()
{ //(RN)
   dReg D1;
   CHARDESC *pcA0;
   RecordName objD7;
...
{ //(i16)
   ...
   RecordName objD4;
   // i16 w_4;
   RecordName origObjInWeaponHand = RNnul;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   // w_4 = 0;
   if(hand < 0)
   {
...
{ //(i16)
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(d.HandChar == -1)
      return 0;                                                                           // owner of cursor
   return ThrowByCharacter(d.HandChar, -1, LeftOrRight, -1, -1, -1, -1, record, x, y, f); // owner of cursor
}
...
{ //()
   ...
   if(D0W != 0)
   {
      ...
      {
         if((d.FacingAlcove == 0) && (d.CellTypeJustAhead == 0)) // Stone or visible FalseWall
         {
            CELLFLAG cf;
            ROOMTYPE rt;
            cf = GetCellFlags(touchedX, touchedY);
            rt = (ROOMTYPE)(cf >> 5);
            if((rt == roomSTONE) || ((rt == roomFALSEWALL) && ((cf & 5) == 0)))
            {
               if((clickX >= 33) && (clickX <= 189) && (clickY >= 44) && (clickY <= 150))
               {
...
bool GravityMove(MouseQueueEnt *ent)
{
   ...
   for(i = 2; i >= 0; i--)
   {
      ...
      if(FindFirstMonster(level, x, y) != RNeof)
         continue;
      switch(axis)
      {
...
{ //(int)
   ...
   //
   thawCommand =
       gameFrozen && (pMouseQueue[d.MouseQStart].num == 148);

   // dragFromMarchingOrder = pMouseQueue[d.MouseQStart].num == 0xffff;

   if(!IsPlayFileOpen() && (lastTime != 0) && (d.Time < lastTime + delta) && (!thawCommand)
      /*&& (RecordFile.IsOpen())*/)
   {
      d.MouseInterlock = 0;
      RETURN_int(result);
   ...
   if((D6W >= 3) && (D6W <= 6) && ((d.partyMoveDisableTimer != 0) || ((d.Word11712 != 0) && (((d.partyFacing + D6W - 3) & 3) == d.Word11714))))
   {
      if(IsPlayFileOpen())
   ...
   {
      D7W = D0W = sw(D6W - 12);
      if(((ui16)D0W < (ui16)d.NumCharacter) && (d.PotentialCharacterOrdinal == 0))
      {
   ...
   {
      if(d.ClockRunning != 0)
         HandleClickInViewport(clickX, clickY);
      lastTime = d.Time;
      RETURN_int(result);
   }
   if(D6W == 81)
   ...
   if(D6W == 140)
   {
      if(((ui16)d.NumCharacter > 0) && (d.PotentialCharacterOrdinal == 0))
      {
         // DisplayDiskMenu(_9_);
