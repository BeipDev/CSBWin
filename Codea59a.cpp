                        ...
                        bool FalseStoneWall)
{
   ...
   while((LOCAL_12 != RNeof) && ((DBType = LOCAL_12.dbType()) <= dbACTUATOR))
   { // Must be a dbTEXT or a dbACTUATOR.
      ...
      if(relativePosition != 0) // Same side as facing?
      {
         ...
         { // Must be an ACTUATOR
            ...
            if((actuatorType != 127) || ((pActuator->State() & PORTRAIT_HideGraphic) == 0))
            {
            ...
            if(D0W == 127)
            {
               if(relativePosition == 2)
               {
                  ...
                  if((pPortrait->State() & PORTRAIT_HidePortrait) == 0)
                  {
                     ...
                     rslt->championPortraitOrdinal = ((dbA2->CastToDB3()->value() & 511) + 1);
                  }
               }
            }
         }
   ...
   for(i = 0; i < 3; i++)
   {
      if((rslt->text[i] != RNeof) && !realDecoration[i])
      {
...
void SummarizeRoomInfo(SUMMARIZEROOMDATA *rslt, i32 facing)
{
   ...
   while((LOCAL_12 != RNeof) && (LOCAL_12.dbType() <= dbACTUATOR))
   {
      LOCAL_12 = GetDBRecordLink(LOCAL_12);
   }
tag00a984:
