...
void settrace(i32 n)
{
   if(n == -1)
   {
      while(timertraceindex > 0)
         timertrace[--timertraceindex] = 0;
   }
   else if(n == -2)
   {
      if(TimerTraceActive)
...
void EnsureItem16Available()
{
   if((d.ITEM16QueLen >= d.MaxITEM16 - 5) && (d.LoadedLevel == d.partyLevel))
   {
      if(d.MaxITEM16 < 240)
      {
         ...
         d.Item16 = (ITEM16 *)UI_realloc(d.Item16, 16 * newMax, MALLOC087);
...
//*********************************************************
// TAG00b1f0
RecordName CreateMonster(i32 value,              // 8
...
{                                          // (RN)
   ...
   if(!MonsterAllowedOnLevel(RNnul, d.LoadedLevel, mt))
      return (RNnul);
                       ...
                       bool (*function)(i32, i32))
{
   ...
   do
   {
      if(DIAG_10) // perfect diagonal
      {
         ...
         if((((*function)(sw(D5W + D7W), D4W) != 0) && ((*function)(D5W, sw(D4W + D6W)) != 0)) || ((*function)(D5W = sw(D5W + D7W), D4W = sw(D4W + D6W)) != 0))
         {
            return 0;
         }
      }
...
i16 Blocked(DB4 *pDB4, i32 individualMonsterIndex, i32 mapX, i32 mapY)
{
   ...
   if((d.Invisible != 0) && !pmtDesc->seesInvisible())
      return 0;
   if(pmtDesc->See360())
      goto tag00c2fe;
   ...
   tag00c2fe: //***
      ...
      if(d.OrthogonalDistance2Party > w_18)
         return 0; // Cannot see the party then zero.
...
i16 GetBestMonsterFacingP1(MONSTERDESC *pmtDesc, i32 mapX, i32 mapY)
{
   ...
   if(((D7W + 1) / 2 >= d.OrthogonalDistance2Party) && (TestDirectPathOpen(mapX,
                                                                           mapY,
                                                                           d.partyX,
                                                                           d.partyY,
...
{                                             // This function expects d.Obj13020 to be the RN of
   ...
   if(D6W >= d.height)
      return false;
   if(monsterMoveInhibit[direction] != 0)
      return false;
   ...
   if(pmtDesc->invincible())
   {
          ...
          obj_2 = GetDBRecordLink(obj_2))
      {
         ...
         if(obj_2.dbType() != dbCLOUD)
            continue;
         pDB15A2 = GetRecordAddressDB15(obj_2);
         if(pDB15A2->cloudType() != CT_FLUXCAGE)
            continue;
   ...
   for(;;)
   {
      ...
      if(D4W != roomTELEPORTER)
         break;
      if((D5W & 8) == 0)
         break; // break if teleporter inactive
      if(pmtDesc->word16_12_15() < 10)
         break;
      DB1A2 = FirstObjectRecordAddressDB1(D7W, D6W);
      if(DB1A2->what() == 0)
         break; // if not monsters
   ...
   d.moveOntoPartyPosition = (d.LoadedLevel == d.partyLevel) && (D7W == d.partyX) && (D6W == d.partyY);
   if(d.moveOntoPartyPosition)
      return 0;
   for(;;)
   {
      if(D4W != roomDOOR)
...
i16 FindPossibleMove(const MONSTERDESC *pmtDesc, i32 x, i32 y, i16 P4)
{
   ...
   {
      if(d.DirectionHasBeenTested[direction] != 0)
         continue;
      D0W = PossibleMove(pmtDesc, x, y, direction, P4);
      if(D0W != 0)
         return sw(direction + 1);
      //
      //
   }
   return 0;
...
{ // P4 means to treat the monsters a pairs.  For example,
   ...
   if(P4)
   {
      if((d.Time == d.Time13130) && (pI16 == d.pI1613134))
         return;
   }
...
//*********************************************************
//   TAG00c8c0
bool MonsterAttacks(RecordName monster,
                    DB4 *pDB4,        // 8
                    i32 mapX,         // 12
                    i32 mapY,         // 14
...
{                                     //(i16)
   ...
   */
   ...
   else
   {
      ...
      rightSideMonster =
          ((attackParameters.monsterPos + 5 - attackParameters.directionToParty) & 2) / 2;
      // 1 if monster is on right side of group when facing party.
   }
   attackParameters.missileOriginPosition = (I16)((rightSideMonster + attackParameters.directionToParty) & 3);
   ...
   //================================== monsterShouldSteal
   attackParameters.monsterShouldSteal = (mtD7 == mon_Giggler);
   ...
   {
      attackParameters.heroToDamage = GetCharacterToDamage(
   ...
   //========================================= missileRange
   attackParameters.missileRange = sw(pmtDesc->attack10 / 4 + 1); // uByte8[4] / 4 + 1);
   attackParameters.missileRange += STRandom(attackParameters.missileRange);
   attackParameters.missileRange += STRandom(attackParameters.missileRange);
      ...
      switch(attackParameters.heroToDamage)
      {
         ...
         case 6: // Closest party member
            attackParameters.heroToDamage = GetCharacterToDamage(
                ...
                attackParameters.monsterY,
                attackParameters.missileOriginPosition);
            if(attackParameters.heroToDamage < 0)
               return false;
            break;
...
void SetMonsterTimerB(TIMER *pTimer, const ui32 AltTime)
{
   ...
   if(AltTime < pTimer->Time())
   {
      switch(pTimer->Function())
      {
         ...
         default:
            die(0xf5af, "Illegal Monster AI Timer");
            tt = TT_MONSTER_B0;
            break;
      }
      pTimer->Function(tt);
...
{ // Timer types 29 to 41
   ...
   while(timerSearch.FindNextTimer())
   {
      ...
      D0L = pTimer->Level();
      if(D0L != d.LoadedLevel)
         continue;
      tt = pTimer->Function();
      if(tt < TT_FIRST_MONSTER_AI)
         continue;
      if(tt > TT_LAST_MONSTER_AI)
         continue;
...
{                                                                                // SMT
   ...
   {                                                                             // SMT
      fprintf(GETFILE(TraceFile),                                                // SMT
              ...
              i_60,
              timeUntilAlternateUpdate,
              processingATimer,                // SMT
   ...
   {                                                               // MDF6TI
      ...
      {                                                            // MDF6TI
         ...
         TurnMonster(pI16A2,                                       // MDF6TI
                     D5W,
                     /*preferred direction*/ // MDF6TI
                     monIdx,
                     /*monster index*/             // MDF6TI
      ...
      {                                     // MDF6TI
         ...
         default:
            die(0x3ffa, "Illegal Monster Index"); // MDF6TI
...
{                                                                       // IC
   ...
   {                   // IC
      case TT_M3:
         timer_70.Function(TT_29); // IC
         break;                    // IC
      case TT_M2:
         timer_70.Function(TT_30); // IC
         break;                    // IC
      case TT_M1:
         timer_70.Function(TT_31);        // IC
         break;                           // IC
...
void ProcessPostMonsterMoveFilter(MONSTERMOVEFILTERLOCATION *pmmfloc,
                                  MMRECORD *pmmr)
{
   if(!pmmr->CalledPreMoveFilter)
      return;
   {
      NEWDSAPARAMETERS ndp;

      if((pmmr->locr.l != pmmr->finalLocr.l) || (pmmr->locr.x != pmmr->finalLocr.x) || (pmmr->locr.y != pmmr->finalLocr.y))
      {
...
{                                 //()
   ...
   i16 yDistance;                                           // w_32;
   i16 xDistance;                                           // w_30;
   const RecordName monster = FindFirstMonster(mapX, mapY); // 28
   ...
   if((timeFunc > 0) && (pmmfloc->filterObj != RNeof) && ((pmmfloc->partyLevelOnly == 0) || (d.LoadedLevel == d.partyLevel)) && ((mapX - d.partyX) * (mapX - d.partyX) + (mapY - d.partyY) * (mapY - d.partyY) <= pmmfloc->maxDistance * pmmfloc->maxDistance))
   {
      NEWDSAPARAMETERS ndp;
   ...
   { // Process Timers of type A
      ...
      switch(timeFunc)
      {
         ...
         default:
            die(0xd7d6, "Illegal Monster AI Timer");
            tt = TT_MONSTER_B0;
            break;
      }
   ...
   if(timeFunc != TT_MONSTER_Bgroup)
   {
      ...
      else // not atacking
      {
         ...
         //
         //
         if((indexOfLastMonster == 0) && (horizSize != 2) && (((D5W = (I16)((i16)STRandom())) & 0xc0) == 0))
         {
            TIMERTRACE(0xd94c);
            ...
            if(((D5W & 0x38) == 0) && (attackDistance != 1) && (horizSize == 0))
            {
               TIMERTRACE(0xd9a2);
                                      ...
                                      || ((TwoBitNibble(pI16A2->facings(), D4W)) == PreferredDirection)))
         {
            ...
            if((attackDistance <= D5W) && ((xDistance == 0) || (yDistance == 0)) && (D5W <= (w_100 = (I16)(STRandom(16) + 1))))
            {
               ...
               if(D5W == 1)
               {
                  ...
                  //
                  //
                  if((!mtDescLocal.word2Bit3() || (STRandom0_3() == 0) || !mtDescLocal.word2Bit4()) && (horizSize == 0) && (pI16A2->positions() != 255))
                  {
                     TIMERTRACE(0xda74);
                     ...
                     if(PreferredDirection != D5W)
                     {
                        ...
                        if(D5W != ((PreferredDirection + 1) & 3))
                        {
                           ...
                           else
                           {
                              ...
                              //
                              //
                              if((D0W == 0) || ((STRandomBool() != 0) && ((OrdinalOfMonsterAtPosition(DB4A3, D5W = (I16)((D5W + 2) & 3)))) == 0))
                              {
                                 TIMERTRACE(0xdb08);
                                 D0W = CheckMonsterVersusMissiles(mapX, mapY, pI16A2->positions(), &mmr);
   ...
   if((fearFactor != StateOfFear0) && (fearFactor != StateOfFear2) && (fearFactor != StateOfFear3))
   {
      // tag00d490:
      TIMERTRACE(0xd490);
      if(AITraceActive)
...
{ //(i16)
   ...
   do
   {
      ...
      {
         if(objD6.dbType() != dbMISSILE)
            continue;
         if(objD6.pos() != position)
            continue;
...
{ //(i16)
   ...
   if(d.PartySleeping == 0)
   {
      ...
      if((D0W >= D1W) && (STRandom0_3() != 0))
         return 0;
      // character is slower (or monster is lucky!)
      if(IsCharacterLucky(chA2, 60))
         return 0;
   }
   D6W = (i16)STRandom();
   if(D6W & 0x70)
...
void DeleteMonsterInGroup(const LOCATIONREL &locr, ui32 indexInGroup)
{
   ...
   if(pMonster->numMonM1() == 0)
      return; // Only one monster there!
   if(pMonster->numMonM1() < indexInGroup)
      return;
   if(AITraceActive)
   {
      fprintf(GETFILE(TraceFile),
   ...
   if(pMonster->fear() == StateOfFear6)
   {
      ...
      while(timerSearch.FindNextTimer())
      {
         ...
         if(pTimer->Level() != (ui16)locr.l)
            continue;
         if(pTimer->timerUByte6() != locr.x)
            continue;
         if(pTimer->timerUByte7() != locr.y)
            continue;
         tt = pTimer->Function();
         if(tt < TT_MONSTER_A0)
            continue;
...
void InsertMonsterInGroup(const LOCATIONREL &locr, ui32 positionMask)
{
   ...
   else if(monsterSize == 1)
   {
      maxNumMon = 2;
      positionMask &= 0x05;
   } // Only 0 and 2 allowed
   else
      return;                       // No room
   numMonM1 = pMonster->numMonM1(); // #monsters-1
   if(numMonM1 + 1 >= maxNumMon)
      return; // No Room
   positionByte = MonsterPositionByte(pMonster, (i16)locr.l, noItem16);
   ...
   if(newpos == 4)
      return; // No place to put new monster.
   //
   ...
   // pTimer = gameTimers.Timers();
   {
      ...
      while(timerSearch.FindNextTimer())
      {
         ...
         if(pTimer->timerUByte7() != locr.y)
            continue;
         tt = pTimer->Function();
         if((tt != TT_MONSTER_A0) && (tt != TT_MONSTER_B0))
            continue;
