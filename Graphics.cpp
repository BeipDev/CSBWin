...
tag00851c:
   ...
   if(pushedD3 != 0)
   {
      ...
      if(D7L >= 0)
         goto tag00851c;
      longGear(dst) = LE32(D2L);
      dst += 4;
      longGear(dst) = LE32(D3L);
      dst += 4;
      goto tag008518;
   }
   longGear(dst) = LE32(D2L);
...
ui8 *FindMemoryBuffer(ui8 *addr, ui8 *&end)
{
   ...
   for(chIdx = 0; chIdx < 4; chIdx++)
   {
      ...
      if(addr == (ui8 *)d.CH16482[chIdx].portrait)
         return (ui8 *)d.CH16482[chIdx].portrait;
   }
   return NULL;
}
#endif
...
//                  8      12      16     20     22     24     26     28
{
   ...
   if(D5W < 0)
      D5W += 16; // if source is left of dest in current word
...
tag008a32:
   ...
   tag008a62:
      ...
      */
      if(H1Valid)
         D1UH2 = (ui16)((LE16(*(puwA0++)) * 0x10001) >> D5W);
      D6UH1 = (ui16)D1UH2;
      D1UH2 &= (ui16)~D4UH2;
      D0UH1 &= (ui16)D4UH2;
      D1UH2 |= (ui16)D0UH1;
      ...
      /*
        ...
        */
      if(H1Valid)
         D2UH2 = (ui16)((LE16(*(puwA0++)) * 0x10001) >> D5W);
      D6UH1 = D2UH2;
      D3UH1 &= D4UH2;
      D2UH2 &= ~D4UH2;
      D2UH2 |= D3UH1;
       ...
       */
      if(H1Valid)
         D3UH2 = (ui16)((LE16(*(puwA0++)) * 0x10001) >> D5W);
      D6UH1 = D3UH2;
      D3UH2 &= ~D4UH2;
      D2UH1 &= D4UH2;
      D3UH2 |= D2UH1;
...
tag008aba:
   D0W = LE16(wordGear(A0));
   A0 += 2;
   // D0W = ((*((ui16 *)A0)) * 0x10001) >> 8; A0+=2;
   D1W = LE16(wordGear(A0));
   A0 += 2;
   // D1W = ((*((ui16 *)A0)) * 0x10001) >> 8; A0+=2;
   D2W = LE16(wordGear(A0));
   A0 += 2;
   // D2W = ((*((ui16 *)A0)) * 0x10001) >> 8; A0+=2;
   D3W = LE16(wordGear(A0));
   A0 += 2;
...
tag008ac2:
   ...
   wordGear(A1) |= LE16(D0W);
   A1 += 2;
   wordGear(A1) &= LE16(D4W);
   wordGear(A1) |= LE16(D1W);
   A1 += 2;
   wordGear(A1) &= LE16(D4W);
   wordGear(A1) |= LE16(D2W);
   A1 += 2;
   wordGear(A1) &= LE16(D4W);
...
i32 LocateNthGraphic(i32 n) // TAG021714
{
   ...
   for(i32 i = 0; i < n; i++)
      D0L += d.GraphicCompressedSizes[i];
   return D0L;
}
...
void openGraphicsFile() // TAG021d36
{
   ...
   if(n == 0)
   {
      ...
      if(m != 0)
      {
         if((graphicSignature1 != expectedGraphicSignature1) || (graphicSignature2 != expectedGraphicSignature2))
         {
            char msg[80];
   ...
   if((EDBT_GraphicsSignature_data != 0xffffffff) && (EDBT_GraphicsSignature_data != 0))
   {
...
void OpenCSBgraphicsFile()
{
   ...
   if((n == 0) && (m != 0))
   {
      if((CSBgraphicSignature1 != expectedCSBgraphicSignature1) || (CSBgraphicSignature2 != expectedCSBgraphicSignature2))
      {
         char msg[80];
...
i16 LZWGetNextCodeword(i16 fileHandle, i32 *pGraphicSize) // TAG022d90
{
   ...
   if((d.Word23382 > 0) | (d.LZWBitNumber >= d.Word23406) | (d.LZWNextCode > d.LZWMaxCode))
                    ...
                    i32 mallocID)
{
   ...
   if((sentinel[0] != 'C') || (sentinel[1] != 'S') || (sentinel[2] != 'B') || (sentinel[3] != 'g'))
   {
      if(firstTime)
      {
   ...
   {
      if(pSectionHeader[i].id != id)
         continue;
      if(pSectionHeader[i].type == (ui32)type)
         break;
   }
   if(i == numSection)
   {
      ...
      {
         char message[100];
         sprintf(message, "Missing section in CSBgraphics.dat.\nSection id=%d, Type=%s", id, CSBGraphicsSectionNames[type]);
   ...
   if(pActualSize != NULL)
      *pActualSize = pSectionHeader[i].uncompressedSize;
   return result;
}
