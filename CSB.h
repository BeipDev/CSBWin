...
struct SUMMARIZEROOMDATA
{
   ...
   RecordName text[3]; // right, front, left
};
...
class DBCOMMON
{
   ...
   friend struct DB14;
   friend struct DB15;
   friend void ConvertListOfObjects(RecordName *, bool, i32 level, i32 x, i32 y);
   // public:
   RecordName m_link;
   void swapLink();

public:
   const RecordName link();
   RecordName *pLink();
   void link(RecordName rn);
...
{ // Door
   ...
   friend void DumpDB0(FILE *f, RecordName object, CELLFLAG cf);
...
{ // Teleporter
   friend void DumpDB1(FILE *f, RecordName object, CELLFLAG /*cf*/);
   friend struct DATABASES;
   void swap();
...
{
   friend void DumpDB3(FILE *, RecordName, i32, i32, i32, CELLFLAG);
   friend struct DATABASES;
   void swap();
...
public:
   ...
   i16 targetX()
   {
      ASSERT((word4 & 0x800) == 0, "No Target");
      return (i16)BITS6_10(word6);
   };
   i16 targetY()
   {
      ASSERT((word4 & 0x800) == 0, "No Target");
      return (i16)BITS11_15(word6);
   };
   i16 targetPos()
   {
      ASSERT((word4 & 0x800) == 0, "No Target");
      return (i16)BITS4_5(word6);
   };
   i16 localActionType()
   {
      ASSERT((word4 & 0x800) != 0, "No Target");
      return (i16)BITS4_15(word6);
   };
...
class DB4 : public DBCOMMON // Monsters??
{
   friend struct DATABASES;
   void swap();
   RecordName possession2; // Swapped when read
...
public:
   ...
   void monsterType(i32 m)
   {
      ASSERT(m < 27, "Illegal Monster Type");
      uByte4 = (ui8)((m & 0x1f) | (uByte4 & 0xe0));
   };
   i32 alternateGraphics() { return (uByte4 >> 5) & 0x7; };
   void alternateGraphics(i32 g) { uByte4 = (ui8)((uByte4 & 0x1f) | ((g & 7) << 5)); };
   RecordName possession() { return possession2; };
   RecordName *pPossession() { return &possession2; };
   ...
   void possession(RecordName object) { possession2 = object; };
...
{
   friend struct DATABASES;
   friend void DumpDB5(FILE *f, RecordName object, i32 /*level*/);
   void swap();
...
class DB6 : public DBCOMMON // Sandals, robe, shield
{
   friend struct DATABASES;
   void swap();
   friend void DumpDB6(FILE *f, RecordName object, i32 /*level*/);
...
public:
   ...
   RecordName text() { return m_RNtext; };
   void text(RecordName text) { m_RNtext = text; };
...
{ // potion??
   friend struct DATABASES;
   friend void DumpDB8(FILE *f, RecordName object, i32 /*level*/);
   void swap();
...
public:
   const RecordName contents() { return m_contents; };
   RecordName *pContents() { return &m_contents; };
   void contents(RecordName rn) { m_contents = rn; };
...
{
   friend void DumpDB10(FILE *f, RecordName object, i32 /*level*/);
   friend struct DATABASES;
   void swap();
...
public:
   ...
   void miscType(MISCTYPE t)
   {
      ASSERT(t < 53, "Illegal Misc Type");
      SETWBITS0_6(word2, t);
   };
   bool important() { return BITS7_7(word2) != 0; };
...
struct DB12 : public DBCOMMON // Not used
{
};

struct DB13 : public DBCOMMON // Not used
{
};

struct DB14 : public DBCOMMON // Fireball.  Other missiles.  Thrown objects.
{
   RecordName flyingObject() { return m_obj2; };
   void flyingObject(RecordName rn) { m_obj2 = rn; };
   RecordName *pFlyingObject() { return &m_obj2; };
...
private:
   RecordName m_obj2;     // 2 missile object dagger, fireball, etc
                          //
...
{
   int object{RecordName(RNnul).ConvertToInteger()};
...
bool GetExtendedCellFlag(i32 level, i32 x, i32 y, EXPOOL_DATA_TYPE ftype);
...
struct EXPOOL
{
   ...
   bool CheckChampionBonesRecord(const RecordName &bonesObject);
   bool CopyChampionBonesRecord(const RecordName &bonesObject, ui32 *pRecord);
   bool GetChampionBonesRecord(const RecordName &bonesObject, ui32 *pRecord);
   void AddChampionBonesRecord(const RecordName &bonesObject, ui32 fingerprint);
...
struct DATABASES
{
   ...
   RecordName AllocateSegment(i32 dbNum, DBTYPE dbType);
   void SetAddress(DBTYPE dbType, DBCOMMON *m);
   DBCOMMON *GetCommonAddress(RecordName obj);
   ...
   RecordName FindEmptyDBEntry(DBTYPE dbType, bool important);
   void swap(i32 dbNum);
...
// This structure is read from disk, do not rearrange!
struct OBJDESC
{
private:
...
public:
   TIMER()
   {
      m_timerSequence = m_timerUByte5 = m_timerUByte6 = m_timerUByte7 = m_timerUByte8 = m_timerUByte9 = m_timerLevel = unused13[0] = unused13[1] = unused13[2] = 0xcd;
   };
...
public:
   ...
   RecordName &timerObj8()
   {
      ASSERT((timerFunction == TT_24) || (timerFunction == TT_25) || (timerFunction == TT_60) || (timerFunction == TT_61) || (timerFunction == TT_ViAltar) || (timerFunction == TT_ReactivateGenerator), "Illegal Timer Type");
      return *(RecordName *)(&m_timerUByte8);
   };
   void swapTimerWord6();
   void swapTimerObj6();
   void swapTimerWord8();
   ...
   RecordName &timerObj6()
   {
      ASSERT((timerFunction == TT_Missile0) || (timerFunction == TT_Missile), "Illegal Timer Type");
      return *(RecordName *)(&m_timerUByte6);
   };
   RecordName *pTimerObj6() { return (RecordName *)&m_timerUByte6; };
   i16 &timerWord6()
   {
   ...
   RecordName *pTimerObj8() { return (RecordName *)&m_timerUByte8; };
};
...
public:
   ...
   SKILL skills92[20]; // 92 //0 and 4-7  =Fighter
                       ...
                       // 3 and 16-19=Wizard
private:
   RecordName possessions[30]; // 212
public:
   inline RecordName Possession(int i) { return possessions[i]; };
   void SetPossession(int i, RecordName obj, bool processFilter);
...
public:
   ...
   TIMER timer;
   RecordName objMonster;
   DSAVARS dsaVars;

   MMRECORD()
   {
      flgs[0] = flgs[1] = 0;
      CalledPreMoveFilter = false;
   };
...
void TAG0010ae();
OBJ_NAME_INDEX GetBasicObjectType(RecordName); // TAG00114c
// OBJ_NAME_INDEX GetObjectNameIndex(RecordName);//TAG001188
...
//   TAG0051a4
void DrawRoomObjects(const RecordName firstObject,
                     const i32 facing,
                     const i32 mapX,
                     const i32 mapY,
                     i32 relativeCellNum,
...
bool MonsterAllowedOnLevel(RecordName monster,
                           i32 level,
                           MONSTERTYPE mt = mon_undefined); // TAG009154
i32 GetObjectWeight(RecordName obj);                        // TAG0091e6
// OBJ_DESC_INDEX  GetObjectDescIndex(RecordName DBRecordName);//9308
i32 GetGraphicClass(RecordName obj); // TAG0093d4(RecordName obj);
i32 TAG009470(CLOTHINGDESC *, bool scale);
i16 MonsterPositionByte(DB4 *pDB4, i16 level, bool noItem16); // TAG0094da
//  i16  GetMonsterDescWord2(RecordName monster);//TAG0094ae
...
DBCOMMON *GetCommonAddress(RecordName object); // TAG009984
DB0 *GetRecordAddressDB0(RecordName object);
DB1 *GetRecordAddressDB1(RecordName object);
DB2 *GetRecordAddressDB2(RecordName object);
DB3 *GetRecordAddressDB3(RecordName object);
DB4 *GetRecordAddressDB4(RecordName object);
DB5 *GetRecordAddressDB5(RecordName object);
DB6 *GetRecordAddressDB6(RecordName object);
DB7 *GetRecordAddressDB7(RecordName object);
DB8 *GetRecordAddressDB8(RecordName object);
DB9 *GetRecordAddressDB9(RecordName object);
DB10 *GetRecordAddressDB10(RecordName object);
DB14 *GetRecordAddressDB14(RecordName object);
DB15 *GetRecordAddressDB15(RecordName object);
...
WEAPONDESC *TAG0099d2(RecordName obj);
RecordName GetDBRecordLink(RecordName recName); // TAG009a02
i16 GetObjectListIndex(i32 mapX, i32 mapY);     // TAG009a1e
RecordName FindFirstObject(i32 mapX, i32 mapY); // TAG009a94
RecordName FindFirstObject(i32 level, i32 x, i32 y);
RecordName FindFirstObject(const LOCATIONREL &locr);
RecordName FindFirstTeleporter(i32 level, i32 x, i32 y);
RecordName FindFirstDrawableObject(i32 mapX, i32 mapY); // TAG009ac8
RecordName FindFirstMonsterGenerator(const LOCATIONREL &locr);
SRCHPKT *SearchForObject(RecordName objID);
void AddObjectToRoom(RecordName DBRecordName,
                     RecordName AddAfter,
                     i32 mapX,
                     i32 mapY,
                     MMRECORD *pmmr); // TAG009b08
//   TAG009c4e
void RemoveObjectFromRoom(RecordName P1, i32 mapX, i32 mapY, MMRECORD *pmmr);
void RemoveObjectFromRoom(RecordName P1, const LOCATIONREL &locr);
RecordName MakeMoreRoomInDB(DBTYPE dbType); // TAG009d9e (RecordName)
// RecordName   FindEmptyDBEntry(ui16);//TAG00a0b0
//  replaced by FindEmptyDBnEntry(bool);
RecordName FindEmptyDB2Entry(bool important);  //(RecordName)
RecordName FindEmptyDB3Entry(bool important);  //(RecordName)
RecordName FindEmptyDB4Entry(bool important);  //(RecordName)
RecordName FindEmptyDB5Entry(bool important);  //(RecordName)
RecordName FindEmptyDB6Entry(bool important);  //(RecordName)
RecordName FindEmptyDB8Entry(bool important);  //(RecordName)
RecordName FindEmptyDB10Entry(bool important); //(RecordName)
RecordName FindEmptyDB14Entry(bool important); //(RecordName)
RecordName FindEmptyDB15Entry(bool important); //(RecordName)
void DeleteDBEntry(DBCOMMON *pDB);
RecordName ConstructPredefinedMissile(ui16 missileType);                   // TAG00a16e
...
RecordName FindFirstMonster(i32 x, i32 y); // TAG00ab6e
RecordName FindFirstMonster(i32 level, i32 x, i32 y);
RecordName FindFirstMissile(i32 x, i32 y); // TAG00ab6e
RecordName FindNextMonster(RecordName obj);
RecordName FindNextMissile(RecordName obj);
RecordName FindFirstDoor(i32 x, i32 y);
RecordName FindFirstDoor(i32 level, i32 x, i32 y);
RecordName FindFirstText(const LOCATIONREL &locr);
...
void AttachItem16ToMonster(RecordName P1, i32 mapX, i32 mapY); // TAG00b062
void DetachItem16(i32 item16Index);                            // TAG00b152
//   TAG00b1f0
RecordName CreateMonster(i32, i32, i32 numMonM1, i32, i32, i32, bool, bool); //(RecordName)
void CreateAndDropObject(i32, i32, i32, i32, i32);                           // TAG00b364()
void TAG00b522(RecordName, i32, i32);                                        //()
...
void KillMonster(RecordName RNmon, ui32 indexInGroup);
...
bool MonsterAttacks(RecordName monster, DB4 *, i32 x, i32 y, i16); // TAG00c8c0//(i16)
...
i16 LaunchMissile(
    RecordName object, // TAG00dd90(i16)
    i32 mapX,
    i32 mapY,
    i32 pos,
    i32 dir,
...
void CreateCloud(RecordName, i16, i32, i32, i32, i32, i16, MMRECORD *pmmrF); //()TAG00dea8
void TAG00e156(RecordName);
void ProcessObjectFromMissile(RecordName, RecordName *dest, i32, i32); // TAG00e17a()
i16 DetermineMagicDamage(DB14 *rec, RecordName object);                // TAG00e218
//   TAG00e35a
...
RecordName IsLordChaosHere(i32 mapX, i32 mapY);                     // TAG00f0e8
bool OpenTeleporterPitOrDoor(i32 mapX, i32 mapY);                   // TAG00f132
// void FluxCage(i32, i32); //TAG00f182
RecordName CreateFluxcage(i32 x, i32 y);
...
bool ProcessDSATimer5(RecordName objDSA,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave);
bool ProcessDSATimer6(RecordName objDSA,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave,
                      bool filter,
                      DSAVARS *dsaVars);
bool ProcessDSATimer7(RecordName objDSA,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave);
void ProcessDSAFilter(RecordName objDSA,
                      const TIMER *pTimer,
                      LOCATIONREL locrSlave,
                      bool *pFilterActive,
                      DSAVARS *dsaVars);
...
i16 TeleportMonster(const DB1 *, RecordName object, i16 attachedLevel, bool noItem16); // TAG011a16
RecordName TeleportMissile(const DB14 *pDB14, RecordName object);                      // TAG011b8e
bool Levitating(RecordName);                                                           // TAG011bf6(RecordName); //Any kind of item
void CreateMovementTimer(RecordName, i32, i32, i32, i16);                              // TAG011c3a
i16 CheckMoveFromMissile(i32, i32, i32, i32, RecordName, MMRECORD *pmmr);              //(i16) //TAG011c9a
//   TAG0011f52
i16 MoveObject(const RecordName object, //(i16)
               const i32 oldX,
               const i32 oldY,
               i32 newX,
               i32 newY,
...
//   TAG012d80
RecordName FindObjectOfTypeAtPosition(i32, i32, ui16 pos, RecordName);
...
//   TAG015492
bool DrawModifiedObjectAtLocation(i32 ScreenLocation, RecordName object);
void DrawEightHands();                // TAG015508//()
void ObjectToCursor(RecordName, i16); // TAG0156c4
RecordName RemoveObjectFromHand();    // TAG01576a//(RecordName)
//   TAG0157e8
void AdjustStatsForItemCarried(CHARDESC *,
                               i32,
                               i32,
                               RecordName);
RecordName RemoveCharacterPossession(i32 chIdx, i32 possessionIndex); // TAG015a66(RecordName)
//   TAG015c4c
void AddCharacterPossession(i32 chIdx, RecordName object, i32 place);   //()
...
i16 TAG016382(RecordName object);
...
void ShowChestContents(RecordName object, DB9 *pDB9, i16);                                       // TAG017ce4
...
void DescribeObject(RecordName, i16);        // TAG0183ea
...
DB8 *GetFlaskInHand(CHARDESC *, RecordName *);                     // TAG01c9b2
