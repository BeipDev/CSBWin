...
{
   RecordName objectToLookFor;
   bool found;
   i32 characterIndex;
   i32 locationOnCharacter; //-1 means cursor
   RecordName RNchest;      // if found as direct content of a chest
   RecordName RNprevObject; // if not first in a list;
};

bool SearchList(CHARACTER_SEARCH_PACKET *pPkt, RecordName first)
{ // return true if in list directly (rather than in an object in the list).
   RecordName prevObj, thisObj;
   DBTYPE dbtype;
   bool direct;
   prevObj = RNeof;
   for(thisObj = first;
...
RecordName FindCursorSource(bool doit)
{
   RecordName result;
   if(d.objectInHand == RNnul)
      return RNeof;
...
i32 FindCharacterSource(i32 chIdx, i32 carryLoc, RecordName *object, bool doit)
{
   DBTYPE dbType;
   if(chIdx >= d.NumCharacter)
      return SER_IllegalCharacterIndex;
   if(carryLoc > 29)
      return SER_IllegalCarryLocation;
   *object = d.CH16482[chIdx].Possession(carryLoc);
   if(*object == RNnul)
      return SER_NoObjectAtCarryLocation;
                   ...
                   i32 doit)
{
   ...
   locr.Integer(srcLoc);
   if(!locr.IsValid())
      return SER_InvalidCellLocation;
   ...
   *object = RNeof;
   ...
   {
      if(((1 << thisObj.pos()) & srcPosMsk) == 0)
         continue;
      if(((1 << thisObj.dbType()) & srcObjMsk) == 0)
         continue;
      if(srcDepth == 0)
      {
         *object = thisObj;
   ...
   if(doit)
   {
      ...
      if((ot != dbMONSTER) || (dstType != 1) || (dstLocr.Integer(dstLoc)->l != d.partyLevel) || (locr.l != d.partyLevel))
      {
         ...
         case dbCLOUD:
            ...
            //      (*timerIndex)++, pTimer++)
            {
               ...
               { // Search entire array???? A bit ugly!
                  ...
                  if((pTimer->Function() != TT_25) && (pTimer->Function() != TT_24))
                     continue;
                  if(thisObj != pTimer->timerObj8())
                     continue;
...
i32 FindMonsterSource(i32 srcObjMsk,
                      ...
                      i32 doit)
{
   RecordName thisObj, prevObj;
   RecordName RNmonster;
   DB4 *pMon;
   DBTYPE dbType;
   DBCOMMON *pThisObj, *pPrevObj;
   if(!RNmonster.checkIndirectIndex(srcLoc))
      return SER_IllegalMonsterID;
   RNmonster.ConstructFromInteger(srcLoc);
   if(RNmonster.dbNum() != dbMONSTER)
      return SER_MonsterIsNotMonster;
   ...
   *object = RNeof;
   ...
   if(*object == RNeof)
      return SER_NoSuchObjectOnMonster;
   dbType = object->dbType();
   switch(dbType)
   {
      case dbCLOUD: return SER_NoCloudsPlease;
                    ...
                    i32 doit)
{
   ...
   DBCOMMON *pThisObj, *pPrevObj;
   if(!RNchest.checkIndirectIndex(srcLoc))
      return SER_IllegalChestID;
   RNchest.ConstructFromInteger(srcLoc);
   if(RNchest.dbNum() != dbCHEST)
      return SER_ChestIsNotChest;
   charSrchPkt.objectToLookFor = RNchest;
   SearchCharactersForObject(&charSrchPkt);
   if(charSrchPkt.found)
      return SER_ChestBelongsToCharacter;
   ...
   *object = RNeof;
   ...
   if(*object == RNeof)
      return SER_NoSuchObjectInChest;
   dbType = object->dbType();
   switch(dbType)
   {
      case dbCLOUD: return SER_NoCloudsPlease;
...
i32 FindCharacterDestination(i32 chIdx, i32 carryLoc, RecordName object, bool doit)
{
   // i32 objIdx;
   i32 locMask;
   DBTYPE dbType;
   ...
   if(chIdx >= d.NumCharacter)
      return DER_IllegalCharacterIndex;
   if(carryLoc > 29)
      return DER_IllegalPossessionIndex;
   if(d.CH16482[chIdx].Possession(carryLoc) != RNnul)
      return DER_PossessionSlotNotEmpty;
   // objIdx = GetObjectDescIndex(object);
   locMask = d.ObjDesc[object.DescIndex()].word4; // Possible carry locations
   if((locMask & d.CarryLocation[carryLoc]) == 0)
      return DER_IllegalObjectForCarryLocation;
   if(!doit)
      return DER_OK;
...
i32 CheckObjectToCursor(RecordName object, bool doit)
{
   DBTYPE dbType;
   if(d.objectInHand != RNnul)
      return DER_CursorNotEmpty;
   dbType = object.dbType();
   switch(dbType)
   {
      case dbCLOUD: return DER_NoCloudsPlease;
                        ...
                        bool doit)
{
   ...
   locr.Integer(dstLoc);
   if(!locr.IsValid())
      return DER_InvalidCellLocation;
   if(dstDepth != 0)
      return DER_IllegalDepth;
   p = locr.p;
   m = dstPosMsk & 15;
   if(m != 0)
   {
   ...
   if(doit)
   {
      ...
      {
         case dbMONSTER: {
            if((locr.l != d.partyLevel) || (srcType != 1) || (srcLocr.Integer(srcLoc)->l != locr.l))
            {
               MONSTERDESC *pmtDesc;
               i32 monsterSize;
               DB4 *pDB4;
      ...
      if((object.dbType() != dbMONSTER) || (locr.l != d.partyLevel) || (srcType != 1) || (srcLocr.Integer(srcLoc)->l != locr.l))
      {
         ...
         if(removed == 0)
         {
            ...
            while(timerSearch.FindNextTimer())
            {
               ...
               if(tt > TT_LAST_MONSTER_AI)
                  continue;
               if(pTimerA->timerUByte6() != srcLocr.x)
                  continue;
               if(pTimerA->timerUByte7() != srcLocr.y)
                  continue;
                         ...
                         bool doit)
{
   ...
   if(!RNchest.checkIndirectIndex(dstLoc))
      return DER_IllegalChestID;
   RNchest.ConstructFromInteger(dstLoc);
   if(RNchest.dbType() != dbCHEST)
      return DER_ChestIsNotAChest;
   if(dstDepth != 0)
      return DER_IllegalDepth;
   charSrchPacket.objectToLookFor = RNchest;
   SearchCharactersForObject(&charSrchPacket);
   if(charSrchPacket.found)
      return DER_ChestBelongsToCharacter;
   count = 0;
   pChest = GetRecordAddressDB9(RNchest);
...
i32 FindMonsterDestination(i32 dstLoc,
                           ...
                           bool doit)
{
   RecordName RNmonster, RNpossessions;
   DBTYPE dbType;
   DBCOMMON *pObject;
   DB4 *pMonster;
   if(!RNmonster.checkIndirectIndex(dstLoc))
      return DER_IllegalMonsterID;
   dbType = object.dbType();
   switch(dbType)
   {
      case dbCLOUD: return DER_NoCloudsPlease;
   ...
   if(RNmonster.dbType() != dbMONSTER)
      return DER_MonsterIsNotAMonster;
   if(dstDepth != 0)
      return DER_IllegalDepth;
