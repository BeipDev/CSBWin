...
{ //(i16)
   ...
   AddObjectToRoom(objD6, RecordName(RNempty), mapX, mapY, NULL);
   D0L = d.Time + 1;
...
void LevelUp(i32 chIdx, i32 basicSkill) // Fighter, ninja, etc.
{
   ...
   switch(basicSkill)
   {
      ...
      case 2: // priest
         ...
         if(d.CH16482[chIdx].MaxMana() + maxManaIncrement > MAXMANA)
            d.CH16482[chIdx].MaxMana(MAXMANA);
         else
            d.CH16482[chIdx].IncrMaxMana(maxManaIncrement);
   ...
   if(maxStaminaIncrement == 0)
      maxStaminaIncrement = 1;
   else
      maxStaminaIncrement += STRandom(maxStaminaIncrement / 2) + 1;
...
// ***********************************************************
//  TAG017498
i16 LaunchObject(CHARDESC *pChar, RecordName object, i32 range, i32 damage, i32 decayRate)
{ //(i16)
   dReg D7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   D7W = pChar->facing;
...
// ***********************************************************
//  TAG0174ec
i16 CharacterThrowsSpell(i32 chIdx, RecordName object, i32 range, i32 neededMana)
{ //(i16)
   dReg D1;
   i32 decayRate;
   CHARDESC *pcA3;
...
{ //()
   ...
   if(clickY <= 48)
   {
      if(d.MagicCaster == 0)
      {
         if(((clickX >= 280) && (clickX <= 291)) || (clickX == 513))
         {
            D6W = 1;
         }
         else
         {
            if(((clickX >= 294) && (clickX <= 305)) || (clickX == 514))
            {
               D6W = 2;
            }
            else
            {
               if(((clickX >= 308) && (clickX <= 319)) || (clickX == 515))
                  D6W = 3;
            }
         }
      }
      else
      {
         if(d.MagicCaster == 1)
         {
            if(((clickX >= 233) && (clickX <= 244)) || (clickX == 512))
            {
               D6W = 0;
            }
            else
         ...
         else
         {
            if(d.MagicCaster == 2)
            {
               ...
               {
                  if(((clickX >= 247) && (clickX <= 258)) || (clickX == 513))
                  {
                     D6W = 1;
                  }
                  else
            ...
            else
            {
               if(d.MagicCaster == 3)
               {
                  ...
                  {
                     if(((clickX >= 261) && (clickX <= 272)) || (clickX == 514))
                     {
...
//       TAG01c9b2
DB8 *GetFlaskInHand(CHARDESC *pChar, RecordName *pObject)
{
   dReg D6;
   RecordName objD7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   for(D6W = 2; (--D6W) >= 0;)
   {
      ...
      if(objD7 == RNnul)
         continue;
      if(objD7.NameIndex() != objNI_EmptyFlask)
         continue;
      *pObject = objD7;
...
{ // Substitute character name for '%'.
   ...
   if(pPercent != NULL)
   {
      ...
      for(i = 0; i < 8; i++)
      {
         if(d.CH16482[charIndex].name[i] == 0)
            break;
...
{ //(i16)
   ...
   RecordName obj_2;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(spellParameters.chIdx != -1)
      chIdx = (ui16)spellParameters.chIdx;
   const char *msg = NULL;
      ...
      case 2: // Fireball, for example
         ...
         }
         {
            ...
            CharacterThrowsSpell(
                chIdx,
                // RecordName(3,15,BITS4_9(pSpell->word6)+0x380), //0xff80+type
                temp,
