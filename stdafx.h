...
#define CEexception(n)                                                 \
   ...
   _except(per = GetExceptionInformation(), CaptureExceptionInfo(), 1) \
   {                                                                   \
      PrintExceptionInfo();                                            \
      PostQuitMessage(1);                                              \
   };
...
{
   CntPtrTo() = default;
   CntPtrTo(const CntPtrTo<T> &ptr) noexcept : m_p(ptr.m_p)
   {
      if(m_p)
         m_p->AddRef();
   }
   ...
   CntPtrTo(CntPtrTo<TDerived> &&ptr) noexcept : m_p(ptr.Extract()) {}
   CntPtrTo(T *pTNew) noexcept : m_p(pTNew)
   {
      if(m_p)
         m_p->AddRef();
   }
   ...
   CntPtrTo<T> &operator=(const CntPtrTo<T> &ptr) noexcept
   {
      *this = ptr.m_p;
      return *this;
   }
   CntPtrTo<T> &operator=(CntPtrTo<T> &&ptr) noexcept
   {
      if(m_p)
         m_p->Release();
      m_p = ptr.Extract();
   ...
   T *Extract() noexcept
   {
      ...
      m_p = nullptr;
      return p;
   } // Return pointer without releasing it

   CntPtrTo<T> &operator=(T *pTNew) noexcept
   {
      T *pT = m_p;
      m_p = pTNew; // To prevent release recursion problems
      if(m_p)
         m_p->AddRef();
      if(pT)
         pT->Release();
