...
// *********************************************************
//   TAG0012a0
void DrawNameOfHeldObject(RecordName object)
{
   char *pName = NULL;
...
//
// *********************************************************
bool MonsterAllowedOnLevel(RecordName object, i32 level, MONSTERTYPE mt)
{
   dReg D7;
   MONSTERTYPE mtD6;
   LEVELDESC *ldA2;
...
// *********************************************************
//   TAG009b08
void AddObjectToRoom(RecordName newObj, RecordName prevObj, i32 mapX, i32 mapY, MMRECORD *pmmr)
{
   dReg D0, D1, D4;
   RecordName objD5;
   CELLFLAG *cfA2;
   ...
   for(j = 0; j < 5; j++)
   {
      if((*(--prnA3)).ConvertToInteger() != 0xffff)
         break;
   }
   if(j < 5)
   {
...
{
   RecordName obj;
   if((GetCellFlags(locr) >> 5) == roomSTONE)
      return RNeof;
       ...
       obj = GetDBRecordLink(obj))
   {
      if(obj.dbType() != dbACTUATOR)
         continue;
      if(GetRecordAddressDB3(obj)->actuatorType() == 6)
         break;
   }
   return obj;
}
...
// *********************************************************
// TAG00ab6e
RecordName FindFirstMonster(i32 mapX, i32 mapY)
{
   RecordName objD7;
...
// *********************************************************
//
RecordName FindFirstMissile(i32 mapX, i32 mapY)
{
   RecordName objD7;
   objD7 = FindFirstObject(mapX, mapY);
...
//
// *********************************************************
RecordName FindFirstDoor(i32 mapX, i32 mapY)
{
   RecordName objD7;
   objD7 = FindFirstObject(mapX, mapY);
...
RecordName FindFirstDoor(i32 level, i32 mapX, i32 mapY)
{
   RecordName objD7;
...
// *********************************************************
// TAG00ab6e
RecordName FindFirstMonster(i32 level, i32 mapX, i32 mapY)
{
   RecordName objD7;
   objD7 = FindFirstObject(level, mapX, mapY);
   while((objD7 != RNeof) && (objD7.dbType() != dbMONSTER)) // DBNum
...
{                                      //()
   ...
   {
      //
      if(((D7W & 0x8000) != 0) && (STRandomBool() != 0))
         continue;
      D7W &= 0x7fff;
      if(D7W >= 127)
      {
      ...
      {
         case dbWEAPON:
            objD5 = FindEmptyDB5Entry(false);
            break;
         case dbCLOTHING:
            objD5 = FindEmptyDB6Entry(false);
            break;
         case dbMISC:
            objD5 = FindEmptyDB10Entry(false);
            break;
...
//
// *********************************************************
void TAG00b522(RecordName monster, i32 newX, i32 newY)
{ //()
...
{ //(i16)
   ...
   if(pMonster->hitPoints6[indexInGroup] == 0xffff)
      return NO_MONSTER_DIED;
   D0W = pMonster->hitPoints6[indexInGroup];
   if(TimerTraceActive)
   {
      fprintf(GETFILE(TraceFile), "Monster[%x] at %02x(%02x,%02x) {HP=%x} loses %02x hitpoints\n", indexInGroup, d.LoadedLevel, attackX, attackY, pMonster->hitPoints6[indexInGroup], damage);
   };
   if(D0UW <= damage)
   { // An individual monster has died.
      ...
      { // Another monster in the group still survives.
         ...
         if(pmtDesc->leavesDroppings() && (!pMonster->drawAsSize4() || !drawAsSize4Monsters))
         {
            if(P6 != 0)
            {
         ...
         if(pMonster->fear() == StateOfFear6)
         {
            ...
            while(timerSearch.FindNextTimer())
            {
               ...
               if(pTimer_4->Level() != (ui16)d.LoadedLevel)
                  continue;
               if(pTimer_4->timerUByte6() != D7W)
                  continue;
               if(pTimer_4->timerUByte7() != D6W)
                  continue;
      ...
      if(showDeathCloud)
      {
         if(pMonster->drawAsSize4() && drawAsSize4Monsters)
         {
            if(w_10 == 0)
            {
               CreateCloud(RecordName(RNMonsterDeath), 255, D7W, D6W, D7W, D6W, 255, pmmr);
            }
         }
         else
         {
            CreateCloud(RecordName(RNMonsterDeath), D5W, D7W, D6W, D7W, D6W, w_20, pmmr);
         }
      }
   ...
   else
   {
      if(damage > 0)
      {
         pMonster->hitPoints6[indexInGroup] = uw(pMonster->hitPoints6[indexInGroup] - damage);
      }
      return NO_MONSTER_DIED;
   }
}
...
void KillMonster(RecordName rnMon, ui32 indexInGroup)
{
   ...
   if(pMon->numMonM1() >= indexInGroup)
   {
      ...
      if((pSrchPkt != NULL) && (pSrchPkt->place == PLACE_InDungeon))
      {
...
{ //(i16)
   ...
   if(w_8 != 0)
      return ALL_MONSTERS_DIED; // If the entire group died.
   if(w_6 != 0)
      return SOME_NOT_ALL_MONSTERS_DIED; // If one or more died.
...
// *********************************************************
//   TAG00dea8
void CreateCloud(RecordName P1,
                 i16 size,
                 i32 objX, i32 objY,
...
{ //()
   ...
   // i32         missileX;
   // i32         missileY;
   RecordName objCloud;   // 6
   RecordName objMonster; // 6
   DB4 *pDB4_4;
   i32 damage;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   // D7W = P2;
   ...
   if((P1 == RNFireball) || (P1 == RNPoison) || (P1 == RNLightning))
   {
   ...
   AddObjectToRoom(objCloud, RecordName(RNempty), missileX, missileY, NULL);
   D0L = d.Time;
   if(P1 == RNffe4)
   {
      D1W = 5;
   ...
   if((P1 != RNLightning) && (P1 != RNFireball))
      return;
   // missileX = w_8;
   // missileY = w_10;
   damage = sw(size / 2 + 1);
   ...
   if((d.LoadedLevel == d.partyLevel) && (objX == d.partyX) && (objY == d.partyY))
   {
      if(TimerTraceActive)
      {
   ...
   if(damage <= 0)
      return;
   d.MonsterDamageResult = (i16)DamageMonsterGroup(pDB4_4, objX, objY, damage, 1, false, pmmr);
}

// *********************************************************
//
// *********************************************************
RecordName CreateFluxcage(i32 mapX, i32 mapY)
{
   RecordName rnD5;
   dReg D0;
   DB15 *DB15A0;
...
//
// *********************************************************
void TAG00e156(RecordName missileObject)
{
   DB14 *pDB14;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
// *********************************************************
//  TAG00e218
i16 DetermineMagicDamage(DB14 *rec, RecordName object)
{
   dReg D0, D1, D5, D6;
   DBTYPE dbType;
   // aReg A2;
   ...
   if(object.IsMagicSpell())
      dbType = dbCLOUD;
   else
      dbType = object.dbType(); // missile type
   if(dbType != dbCLOUD)
   {
      if(dbType == dbWEAPON)
      {
...
int MissileEncounterFilter(int type, LOCATIONREL locr, RecordName missile, RecordName projectile, i32 index)
// See if there is a missile encounter filter.
// Return non-zero if encounter is to be modified.
{
   ui32 key;
   int result = 0;
   RecordName missileEncounterDSA;
...
{ //(i16)
   ...
   if(RNprojectile.IsMagicSpell())
      projectileDBType = dbCLOUD;
   else
      projectileDBType = RNprojectile.dbType();
   ...
   nonPoison_Spell = (projectileDBType == dbCLOUD) && (RNprojectile != RNPoison) && (RNprojectile != RNPoisonBolt);
   pobj_24 = NULL;
   D4W = 0;
   // if (XX < 255)
   //{
   ...
   switch(cellType)
   {
      ...
      case roomDOOR: {
         ...
         if(D0W & 2)
         {
            ...
            {
               if(RNprojectile.IsNonPhysicalSpell())
                  D0W = 1;
               else
                  D0W = 0;
            }
      ...
      case -1: // Monster
         ...
         D1L = DeterminePoisonDamage(mt_60, d.Word12962);
         {
            ...
            if(nonPoison_Spell)
               break;
            if(monsterDamageResult != NO_MONSTER_DIED)
               break;
         }
         if(projectileDBType != dbWEAPON)
            break;
         if(!pmtDesc->absorbsDaggersEtc())
            break;
         pDB5_16 = GetRecordAddressDB5(RNprojectile);
         wtD6 = pDB5_16->weaponType();
         if((wtD6 != weapon_Dagger) && (wtD6 != weapon_Arrow) && (wtD6 != weapon_Slayer) && (wtD6 != weapon_PoisonDart) && (wtD6 != weapon_ThrowingStar))
            break;
   ...
   {
      int soundNum;
      if(!RNprojectile.IsMagicSpell() && (RNprojectile.dbType() == dbWEAPON))
      {
         soundNum = 0;
      }
      else
    ...
    i32 newY)
{
   ...
   for(;;)
   {
      ...
      if(D0W != 0)
      {
         ...
         if(D0W == 0)
         {
            if((dirD4 != 0) || (IsItInThisDirection(
            ...
            i32 force, bool explosion, i32 delay)
{
   ...
   //
   if((explosion && !DB0A3->canOpenWithFireball()) || (!explosion && !DB0A3->canOpenWithAxe()))
   {
      return 0;
   }
   if(force < (d.DoorTOC[DB0A3->doorType()] & 0xff))
      return 0;
   cfA2 = &d.LevelCellFlags[mapX][mapY];
...
// *********************************************************
//  TAG011a16
i16 TeleportMonster(const DB1 *pDB1, RecordName object, i16 attachedLevel, bool noItem16)
{
   dReg D0, D4, D5, D6;
   MONSTERTYPE mtD0;
   RecordName objD7;
   ...
   D6W = DB1A3->rotation();
   if(!noItem16)
      D5W = MonsterFacingByte(DB4A2, attachedLevel, noItem16);
   else
      D5W = d.Byte10340[DB4A2->facing()]; // 0000,1111,2222,3333 encoded in 2-bit nibbles
   ...
   if(!noItem16)
      w_2 = MonsterPositionByte(DB4A2, attachedLevel, noItem16);
   else
      w_2 = DB4A2->groupIndex();
   if(w_2 != 255)
   {
      w_8 = w_2;
   ...
   if(!noItem16)
      SetMonsterFacingByte(DB4A2, D4W, attachedLevel, noItem16);
   // 30OCT02else SETWBITS8_9(DB4A2->word14, D4W);
   else
      DB4A2->facing(D4W);
   if(!noItem16)
      SetMonsterPositionByte(DB4A2, w_2, attachedLevel, noItem16);
   else
      DB4A2->groupIndex(w_2);
   if((attachedLevel == d.partyLevel) && (DB4A2->fear() == StateOfFear6))
   {
...
// *********************************************************
//  TAG011b8e
RecordName TeleportMissile(const DB1 *pDB1, RecordName object)
{
   dReg D5, D6;
   RecordName objD7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
// *********************************************************
//  TAG011bf6
bool Levitating(RecordName item)
{
   DBTYPE dbType;
   RecordName objD7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
// *********************************************************
//   TAG011c3a
void CreateMovementTimer(RecordName object,
                         i32 mapX,
                         i32 mapY,
                         i32 level,
                         i16 P5)
{
   ...
   while(timerSearch.FindNextTimer())
   {
      ...
      if(pTimer->Function() != tt)
         continue;
      if(pTimer->timerUByte6() != mapX)
         continue;
      if(pTimer->timerUByte7() != mapY)
         continue;
      // if (pTimer->timerTime >> 24 != (ui32)level) continue;
      if(pTimer->Level() != level)
         continue;
      count++;
   }
   if(count >= 31)
...
// *********************************************************
//  TAG011c9a
i16 CheckMoveFromMissile(i32 oldX, i32 oldY, i32 newX, i32 newY, RecordName movedObject, MMRECORD *pmmr)
{ //(i16)
   ...
   for(; numCellsToTry > 0; numCellsToTry--)
   {
      ...
      do
      {
             ...
             objD7 = FindNextMissile(objD7))
         {
            ...
            TIMERTRACE(tt);
            if(tt == TT_Missile0)
               continue; // Still in thrower's square
...
// *********************************************************
//  TAG011f52
i16 MoveObject(const RecordName object,
               const i32 oldX,
               const i32 oldY,
               i32 newX,
               i32 newY,
...
{ //(i16)
   ...
   if((oldX >= 0) && ((objD7 == RNnul) || ((objectDBTypeD4 == dbMONSTER) && (d.LoadedLevel == d.partyLevel))))
   {
   ...
   if(newX >= 0)
   {
      ...
      { // Limited teleportation??? 1000 seems sufficient.
         ...
         D5W = sw(D6W >> 5);
         if((D5W == roomTELEPORTER) && ((objectDBTypeD4 < 0) || (objectDBTypeD4 >= dbMONSTER)))
         {
            if((D6W & 8) == 0)
               break;
            if(pPretendTeleporter != NULL)
            ...
            if(DB1A2 == NULL)
               break;
            // D0W = DB1A2->word2;
            if((DB1A2->what() == 1) && (objectDBTypeD4 != dbMONSTER))
               break; // monsters only
            ...
            { // Monster or party entering teleporter
               ...
               if(DB1A2->what() == 0)
                  break; // Objects Only
               if((DB1A2->what() == 1) & (monsterPartyOther != MPO_monster))
                  break;
               if((DB1A2->what() == 2) & (monsterPartyOther != MPO_party))
                  break;
            }
            ...
            if(TimerTraceActive)
            {
               fprintf(GETFILE(TraceFile), "Teleporting object %04x from %02x(%02x,%02x)", object.ConvertToInteger(), curLevel, newX, newY);
            }
            newX = DB1A2->destX();
            newY = DB1A2->destY();
         ...
         else // not teleporter
         {
            if((D5W == roomPIT)    // Room type = PIT
               ...
               && ((D6W & 1) == 0) && PitGoesSomewhere(curLevel, newX, newY) && ((objectDBTypeD4 < 0) || (objectDBTypeD4 >= dbMONSTER)))
            {
               if((LOCAL_20 != 0) && (d.SupressPitDamage == 0))
               {
                  LOCAL_20 = 1; // d.BigMemory;
               ...
               if(TimerTraceActive)
                  fprintf(GETFILE(TraceFile), "Object %04x falls from %02x(%02x,%02x)", object.ConvertToInteger(), curLevel, newX, newY);
            ...
            else
            {
               ...
               if(objectDBTypeD4 == dbMISSILE)
                  break;
               if((objectDBTypeD4 >= 0) && (objectDBTypeD4 < dbMONSTER))
                  break;
               if((D6W & 4) == 0)
               {
                  curLevel = IncrementLevel(curLevel, 1, &newX, &newY);
      ...
      {
         if((LOCAL_18 != 0) || !MonsterAllowedOnLevel(objD7, curLevel))
         {
            ...
            ASSERT(AttachedLevel_4 >= 0, "attachedLevel");
            if(oldX >= 0)
               ProcessMonsterDeleteFilter(oldX, oldY, &dmp, AttachedLevel_4);
      ...
      if(unchangedRoom)
      {
         ...
         else
         {
            ...
            if((d.NewPos == objectPosition) && (objectDBTypeD4 != dbMISSILE))
            {
               return 0;
            }
            // tag012664:
      ...
      else
      {
         ...
         if((objD7 == RNnul) && (d.NumCharacter != 0))
         {
            LOCAL_28 = D6W;
            ...
            d.PartyFootprints[D6W] = sw(((curLevel & 0x3f) << 10) | ((newY & 0x1f) << 5) | (newX & 0x1f));
            d.Byte13220[D6W] = 0;
   ...
   if(objectDBTypeD4 == dbMONSTER)
   {
      ...
      if(((curLevel == d.partyLevel) && (newX == d.partyX) && (newY == d.partyY)) || ((objD0 = FindFirstMonster(newX, newY)) != RNeof))
      // tag0128aa:
      ...
      if(item16Attached && (curLevel != d.partyLevel))
      {
         DetachItem16(D6W);
         LOCAL_14 = 1;
         // goto tag012956;
      ...
      if(levitated)
      {
         AddObjectToRoom(objD7, RecordName(RNempty), newX, newY, pmmr);
         //    goto tag012988;
      }
      else
      {
   ...
   if(objectDBTypeD4 == dbMISSILE)
   {
      AddObjectToRoom(objD7, RecordName(RNempty), newX, newY, NULL);
   }
   else
   {
      SET(D0B, d.LoadedLevel == d.partyLevel);
...
void RotateActuatorList()
{
   DBCOMMON *dbA3;
   DB3 *DB3A2;
   RecordName objD6, objD7;
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   if(d.DelayedActuatorAction == -1)
      return;
   if((d.DelayedActuatorAction == 1) || (d.DelayedActuatorAction == 2))
   {
      objD7 = FindFirstObject(d.DelayedActuatorActionX,
                              d.DelayedActuatorActionY);
      while((objD7.dbType() != dbACTUATOR) || ((d.DelayedActuatorActionPos != -1) && (objD7.pos() != (unsigned)d.DelayedActuatorActionPos)))
      {
         objD7 = GetDBRecordLink(objD7);
         //
         //
      ...
      objD6 = dbA3->link();
      while((objD6 != RNeof) && ((objD6.dbType() != dbACTUATOR) || ((d.DelayedActuatorActionPos != -1) && (objD6.pos() != (unsigned)d.DelayedActuatorActionPos))))
      {
      ...
      if(objD6 != RNeof)
      {
         ...
         {
            //
            if((d.DelayedActuatorActionPos == -1) || (objD6.pos() == (unsigned)d.DelayedActuatorActionPos))
            {
               DB3A2 = GetRecordAddressDB3(objD6);
            }
            objD6 = GetDBRecordLink(objD6);
...
{ //()
   ...
   else
   {
      ...
      if((targetRoomType == roomSTONE) || (targetRoomType == roomFALSEWALL))
      {
      ...
      if(chA3->HP() != 0)
      {
         ...
         tag012e14:
                ...
                objD4 = GetDBRecordLink(objD4))
            {
               ...
               if(GetBasicObjectType(objD4) == objNID7)
                  return 1;
               //
               //
            }
...
{ // I broke this out of 13380 to make it more manageable.
   ...
   { // Actuator in open room - like pressure pad
      switch(DB3A3->actuatorType())
      {
         ...
         case 4:
            ...
            if(ActuatorValue != movedObjectNameIndex)
               return; // no action
            // D0W = LOCAL_12;
            // Check if pad was already pressed by such an object.
            if(sameTypeObjectPresent)
               return; // no action
            break;
         case 5:
            return; // no action
         ...
         case 9:
            if(movedObjectDBNum != -1)
               return; // no action
            if(!place)
               return; // if object is being removed then no action
            if(unchangedRoom)
               return; // no action
   ...
   { // Actuator in Stone wall.  Like lever.  Position of
      ...
      if((unsigned)switchPosition != objD6.pos())
         return; // no action
   ...
   if(TimerTraceActive)
   {
      fprintf(GETFILE(TraceFile), "Object %04x stepped %s pressure pad %04x ", object.ConvertToInteger(), place ? "On" : "Off", objD6.ConvertToInteger());
...
void PlaceOrRemoveObjectInRoom(
    i32 mapX,
    i32 mapY,
    RecordName object,  // RNnul refers to the party
...
{ // currentRoom is 1 if object being placed in same room as party
   ...
   { // Not in stone
      ...
      while(objD6 != RNeof)
      {
         ...
         {
            if((DBType == dbTEXT) && (movedObjectDBType == -1) && (place) && (!unchangedRoom))
            {
               char text[1000];
               ...
               // text[0] = 0x0a;
               if(DecodeText(text, GetRecordAddressDB2(objD6), 1, 990) != -1)
               {
            ...
            {
               if((DBType > dbMONSTER) && (DBType < dbMISSILE))
               {
   ...
   { // in stone cell
      while(objD6 != RNeof)
      {
         if(((i32)objD6.pos() == positionOfObject) && (objD6.dbType() > dbMONSTER))
         {
            objectDB5To13Present = true;
            objNID0 = GetBasicObjectType(objD6);
   ...
   {
      AddObjectToRoom(object, RecordName(RNempty), mapX, mapY, pmmr);
   }
...
{ // ScreenLocation = 0 to 7 for the portraits
   ...
   //
   //
   if(((objNID6 >= objNI_FirstModifiableObject) && (objNID6 <= objNI_LastModifiableObject)) || ((objNID6 >= objNI_FirstFullFlask) && (objNID6 <= objNI_LastFullFlask)) || (objNID6 == objNI_EmptyFlask))
   {
...
{ //()
   ...
   //
   if((d.PotentialCharacterOrdinal != 0) && (D6W == 0))
      return;

   d.Word16608 = 0;
   objNID5 = (OBJ_NAME_INDEX)d.ObjectType16574;
   if(objNID5 > objNI_LastModifiableObject)
      goto tag01553c;
   // otD5 = (OBJECTTYPE)d.Word16574;
   if(objNID5 >= objNI_FirstModifiableObject)
      goto tag015560;
tag01553c:
   objNID5 = (OBJ_NAME_INDEX)d.ObjectType16574;
   if(objNID5 < objNI_FirstFullFlask)
      goto tag015554;
   // D5W = d.Word16574;
   if(objNID5 <= objNI_LastFullFlask)
      goto tag015560;
tag015554:
   objNID5 = (OBJ_NAME_INDEX)d.ObjectType16574;
   if(objNID5 != objNI_EmptyFlask)
      goto tag0155a8;
tag015560:
   ASSERT(RememberToPutObjectInHand == -1, "objInHand");
