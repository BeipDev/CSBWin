                  ...
                  bool dieIfError)
{
   ...
   //    };
   //  };
   minSize = sizeof(CSB_BITMAPFILEHEADER) + sizeof(CSB_BITMAPINFOHEADER) + sizeof(*pBmiColors),
   tm.m = ReadCSBgraphic(
       CGT_Portrait,
       index,
       minSize,
   ...
   numColor = 16;
   if(pBmInfoHeader->biClrUsed != 0)
      numColor = pBmInfoHeader->biClrUsed;
...
pnt GetExternalPortraitAddress(i32 index)
{
   ...
   if((bitmapentry.width != 32) || (bitmapentry.height != 29))
   {
...
bool OVERLAYDATA::ReadOverlay(i32 onum)
{
   ...
   if(pBmiHeader->biClrUsed != 0)
      numColor = pBmiHeader->biClrUsed;
   pPixels = (ui8 *)pColors + 4 * numColor;
   bmpsize = (pBmiHeader->biWidth + 1) / 2 * 2;
   bmpsize *= pBmiHeader->biHeight;
   if(fileSize < sizeof(CSB_BITMAPFILEHEADER) + sizeof(CSB_BITMAPINFOHEADER) + 4 * numColor + bmpsize)
   {
      if(!firstTime)
         return false;
...
void SelectOverlay(i32 onum, i32 p1, i32 p2, i32 p3, i32 p4)
{
   ...
   if((currentOverlay.m_p1 != p1) || (currentOverlay.m_p2 != p2) || (currentOverlay.m_p4 != p4))
      currentOverlay.m_change = true;
