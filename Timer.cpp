...
void ProcessPortraitTimer7(RecordName targetObj, const TIMER *pTimer);
...
void TraceTimer(TIMER *pTimer, i32 index, const char *msg)
{
   ...
   {
      case 6:
         fprintf(GETFILE(TraceFile), " %s  0x%02x in DB at ", action(timerTypeModifier[pTimer->timerUByte9()]), 1 << pTimer->timerUByte8());
         ...
         break;
      case TT_FALSEWALL:
         fprintf(GETFILE(TraceFile), " %s CELLFLAG 0x04 at %02x,%02x", action(timerTypeModifier[pTimer->timerUByte9()]), pTimer->timerUByte6(), pTimer->timerUByte7());
         break;
      case 8:
         fprintf(GETFILE(TraceFile), " %s CELLFLAG 0x08 at %02x,%02x", action(timerTypeModifier[pTimer->timerUByte9()]), pTimer->timerUByte6(), pTimer->timerUByte7());
         break;
   }
...
//*********************************************************
//   TAG00e17a
void ProcessObjectFromMissile(RecordName missile, RecordName *pDest, i32 mapX, i32 mapY)
{ //()
   RecordName objD6;
   RecordName objD7;
   DB14 *DB14A3;
   ...
   if(!objD6.IsMagicSpell() && (objD6.dbType() != dbCLOUD))
   {
      ...
      {
         MoveObject(
             RecordName(objD6).pos(missile.pos()),
             -2,
             0,
             mapX,
             mapY,
...
// of it.
RecordName missileFilterObject = RNnul;
...
{ //()
   ...
   else
   {
      ...
      if((d.LoadedLevel == d.partyLevel) && (curMapX == d.partyX) && (curMapY == d.partyY))
      {
   ...
   do
   {
      ...
      curPos = missileFilterObject.pos();
      leavingThisCell = (missileDirection == curPos) || (((missileDirection + 1) & 3) == curPos);
      curMapX = origMapX;
      curMapY = origMapY;
      if(leavingThisCell)
      {
         ...
         if((curMapX < 0) || (curMapX >= d.width) || (curMapY < 0) || (curMapY >= d.height))
         {
         ...
         if(nextIsStoneWall || (nextIsFalseWall && falseWallIsSolid) || (nextIsStairs && origIsStairs))
         {
   ...
   else
   {
      ...
      missileFilterObject.pos(curPos);
      RemoveObjectFromRoom(missileFilterObject, curMapX, curMapY, NULL);
      AddObjectToRoom(missileFilterObject, RecordName(RNempty), curMapX, curMapY, NULL);
   }
...
{ //()
   ...
   //};
   if((d.LoadedLevel == d.partyLevel) && (D7W == d.partyX) && (D6W == d.partyY))
      w_16 = 1;
   else
      w_16 = 0;
   // w_16 = D0W & 1;
...
bool OpenTeleporterPitOrDoor(i32 mapX, i32 mapY)
{
   ...
   return (cf == roomOPEN) || (cf == roomTELEPORTER) || (cf == roomPIT) || (cf == roomDOOR);
}
...
bool TIMER::operator<(const TIMER &timer2) const // TAG00fd9e
{
   ...
   if(timerFunction > timer2.timerFunction)
      return true;
   if(timerFunction < timer2.timerFunction)
      return false;
   if(m_timerUByte5 > timer2.m_timerUByte5)
...
HTIMER GameTimers::SetTimer(TIMER *pNewTimer) //
{
   ...
   if((pNewTimer->Function() >= TT_MAP_FIRST) && (pNewTimer->Function() <= TT_MAP_LAST))
   {
      if(deleteDuplicateTimers != 0)
      {
         ...
         while(timerSearch.FindNextTimer())
         {
            ...
            if((ptA2->Function() >= TT_MAP_FIRST) && (ptA2->Function() <= TT_MAP_LAST))
            {
               // if (pNewTimer->timerTime != ptA2->timerTime) continue;
               if(pNewTimer->Time() != ptA2->Time())
                  continue;
               if(pNewTimer->Level() != ptA2->Level())
                  continue;
               if(pNewTimer->timerWord6() != ptA2->timerWord6())
                  continue;
               if(ptA2->Function() == TT_STONEROOM)
               {
                  if(ptA2->timerUByte8() != pNewTimer->timerUByte8())
                     continue;
               }
...
#define TimerSequencePageSize 32
   ...
   do
   {
      ...
      if(m_firstAvailTimer == m_maxTimers)
         break;
...
{
   if(m_searchActive)
      die(0xccb7, "Recursive timer search");
   m_searchActive = true;
...
{ //()
   ...
   if(D5W == 1)
   {
      ...
      if((d.LoadedLevel == d.partyLevel) && (x == d.partyX) && (y == d.partyY) && (D4W != 0))
      {
         if(d.NumCharacter > 0)
         {
...
void ProcessTT_FALSEWALL(TIMER *pTimer, ui32 index) // function = 7
{
   ...
   }
   {
          ...
          obj = GetDBRecordLink(obj))
      {
         ...
         if((actuatorType == 127) && (obj.pos() == pTimer->timerUByte8()))
         {
         ...
         if(actuatorType == 47)
         {
            ...
            locr.y = mapY;
            locr.p = obj.pos();
            if(ProcessDSATimer7(obj, pTimer, locr))
               break;
            continue;
         }
      }
   ...
   if(D5W == 1)
   {
      //
      //
      if((d.LoadedLevel == d.partyLevel) && (mapX == d.partyX) && (mapY == d.partyY))
      {
         // pTimer->timerTime++;
         pTimer->Time(pTimer->Time() + 1);
      ...
      else
      {
         ...
         if((objD4 != RNeof) && objD4.NonMaterial())
         {
            // pTimer->timerTime++;
            pTimer->Time(pTimer->Time() + 1);
            ;
...
void ActivateDSA(int level, int x, int y, int pos, int action)
{
       ...
       objActuator = GetDBRecordLink(objActuator))
   {
      ...
      DB3 *pActuator;
      i32 actuatorType;
      if(objActuator.dbType() != dbACTUATOR)
         continue;
...
void ProcessTT_DOOR(TIMER *pTimer)
{
   ...
   {
      if(D7W == 0)
         pTimer->timerUByte9(1);
      else
         pTimer->timerUByte9(0);
   }
   else
   {
...
{ //()
       ...
       objD4 = GetDBRecordLink(objD4))
   {
      ...
      if((D7W == dbTEXT) && (pTimer->Function() != TT_DESSAGE) && (pTimer->Function() != TT_ParameterMessage))
      {
         ...
         if((D6W == 0) && (pDB2_4->show() != 0) && (d.LoadedLevel == d.partyLevel) && (targetX == d.partyX) && (targetY == d.partyY))
         {
            char text[1000];
            DecodeText(text,
      ...
      else
      {
         if(D7W == dbACTUATOR) // DB3
         {
            ...
            if(pDB3->actuatorType() == 47)
            {
               ...
               locr.y = targetY;
               locr.p = objD4.pos();
               if(ProcessDSATimer5(objD4, pTimer, locr))
                  break;
               continue;
            }
            if(pTimer->Function() == TT_DESSAGE)
               continue;
            if(pTimer->Function() == TT_ParameterMessage)
               continue;
            ...
            {
               if(TimerTraceActive)
                  fprintf(GETFILE(TraceFile), "Actuator6(counter) %03x %02x(%02x,%02x) ", objD4.idx(), d.LoadedLevel, targetX, targetY);
...
void ProcessTT_ReactivateGenerator(TIMER *pTimer)
{
   ...
   // not work well.  Now we store the generator's ID (RecordName id)
   ...
   //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   RecordName generatorID;
   RecordName firstTypeZero;
       ...
       objD7 = GetDBRecordLink(objD7))
   {
      ...
      if(objD7.dbType() == dbACTUATOR)
      {
         ...
         if(DB3A2->actuatorType() != 0)
            continue;
         if(firstTypeZero == RNeof)
            firstTypeZero = objD7;
         if(objD7 == generatorID)
            break;
      }
      //
      //
...
{ //()
   // dReg D5;
   i32 actuatorTypeD4;
   RecordName secondObject, firstObject;
   // i16 w_26;
   i16 objectPosition;
   ...
   T7or8or14 = ((actuatorTypeD4 == 7) || (actuatorTypeD4 == 8) || (actuatorTypeD4 == 14));
   ...
   else
   {
      if((actuatorTypeD4 == 14) || (actuatorTypeD4 == 15))
      {
             ...
             firstObject = GetDBRecordLink(firstObject))
         {
            ...
            {
               if(objectPosition == pTimer->timerUByte8())
                  break; // correct position
               if(objectPosition == ((pTimer->timerUByte8() + 1) & 3))
                  break; // correct position
            }
            //
         }
         ASSERT(firstObject != RNnul, "firstObj");
         if(firstObject == RNeof)
            return; // original had RNnul) return;
...
void ThrowMissile(i32 missileType,
                  ...
                  i32 decayRate)
{
   RecordName objectToThrow;
   DBTYPE objType;
   objectToThrow = RNeof;
   i32 oldLevel;
   if(!launchLocation.IsValid())
      return;
   switch(missileType & 0xffff)
   {
      ...
      case 0: {
             ...
             objectToThrow = GetDBRecordLink(objectToThrow))
         {
            ...
            if((objType > dbMONSTER) && (objType <= dbMISC))
            {
               if(objectLocation.p == objectToThrow.pos())
                  break;
            }
         }
         if(objectToThrow == RNeof)
            return; // original had RNnul) return;
      ...
      else
      {
         if(objectDB == dbACTUATOR)
         {
            ...
            if(actuatorType == 47)
            {
               ...
               if(ProcessDSATimer6(objAtTarget, pTimer, locr, false, &dsaVars))
                  break;
               continue;
            }
            if((actuatorType == 127) && (objAtTarget.pos() == pTimer->timerUByte8()))
            {
            ...
            { // Counter
               if(TimerTraceActive)
                  fprintf(GETFILE(TraceFile), "Actuator6(counter) %03x %02x(%02x,%02x) ", objAtTarget.idx(), d.LoadedLevel, targetX, targetY);
            ...
            else
            {
               ...
               else
               {
                  ...
                  if((((actuatorType >= 7) && (actuatorType <= 10)) || (actuatorType == 14) || (actuatorType == 15)) && (objAtTarget.pos() == pTimer->timerUByte8()))
                  {
                     if(TimerTraceActive)
                        fprintf(GETFILE(TraceFile), "Actuator5 %03x (%02x,%02x) ", objAtTarget.idx(), targetX, targetY);
                     TriggerMissileLauncher(pDB3A2, pTimer);
                     if(pDB3A2->onceOnly())
                        pDB3A2->actuatorType(0);
                  }
                  else
                  {
                     if(actuatorType == 18)
...
{ //()
   ...
   {
      if(*pcf & 0x08)
         pTimer->timerUByte9(tmrAct_CLEAR);
      else
         pTimer->timerUByte9(tmrAct_SET);
   }
...
{ //()
   ...
   if((d.LoadedLevel == d.partyLevel) && (mapX == d.partyX) && (mapY == d.partyY))
   {
      MoveObject(RecordName(RNnul), mapX, mapY, mapX, mapY, NULL, NULL); // move party
      DrawEightHands();
   }
   objD4 = FindFirstMonster(mapX, mapY);
   if(objD4 != RNeof)
       ...
       objD4 = obj_2)
   {
      ...
      else
      {
         ...
         { // Search entire array???? A bit ugly!
            ptA2 = timerSearch.TimerAddr();
            if((ptA2->Function() != TT_25) && (ptA2->Function() != TT_24))
               continue;
            if(objD4 != ptA2->timerObj8())
               continue;
...
{ //()
   ...
   if(D0W == 0)
   {
      ...
      *pcfA2 |= 0x08;
      if(wasClosed)
         WiggleEverything(x, y); // So they will fall through
                                 // new pit (or whatever)
   }
   else
   {
...
{ //()
   ...
   do
   {
      ...
      if(((d.partyLevel != d.LoadedLevel) || (d.partyX != D7W) || (d.partyY != D6W)) && (FindFirstMonster(D7W, D6W) == RNeof))
      ...
      if(pDB4A2->monsterType() != mon_LordChaos)
         break;

      if(STRandom0_3() != 0)
         break; // 25 percent chance of movement.
...
void EnableCharacterAction(i32 chIdx)
{
   ...
   if(pChar->HP() != 0)
   {
      if((pChar->attackType == atk_SHOOT) && (pChar->Possession(0) == RNnul))
      {
         D6W = 12;
...
{ //()
   ...
   {
      case 2:
         CreateCloud(RecordName(RNffe4), 0, bonesX, bonesY, bonesX, bonesY, D5W, NULL);
         // pTimer->timerTime += 5;
         pTimer->Time(pTimer->Time() + 5);
         ViState--;
      ...
      case 1:
             ...
             objD4 = GetDBRecordLink(objD4))
         {
            ...
            if(objD4.pos() != D5UW)
               continue;
            if(objD4.dbType() != dbMISC)
               continue;
            objNI_2 = objD4.NameIndex();
            if(objNI_2 != objNI_Bones_a)
               continue;
            if(objD4 != objBones)
               continue;
            if(expool.GetChampionBonesRecord(objD4, bonesRecord) == 1)
            {
               ...
               {
                  if(d.CH16482[chIndex].fingerPrint == (bonesRecord[0] & 0xffff))
                     break;
               }
               if(chIndex < d.NumCharacter)
               {
                  if(d.CH16482[chIndex].HP() != 0)
                     break; // ignore if character not dead!
               ...
               else
               {
                  ...
                  CHARDESC character;
                  if(!character.GetFromWings((ui16)(bonesRecord[0] & 0xffff), false))
                     break;
                  if(character.HP() > 0)
                     break; // character not dead!
...
void ProcessLightLevelTimer(TIMER *pTimer)
{
   ...
   changeInBrightness = d.Word1074[oldNumberOfSteps] - d.Word1074[newNumberOfSteps];
   if(negativeChange)
   {
