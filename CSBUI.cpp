   ...
   */
   ...
   while(GETS(buf, 500, f) != NULL)
   {
      ...
      if(strcmp(field, "TIMER") == 0)
      {
         field = getfield(buf, col);
         if(TImER)
            continue; // cmd line has higher priority than config.linux
      ...
      if(strcmp(field, "PLACE") == 0)
      {
         ...
         if((width < 1) || (srcx + width > 320))
            continue;
         height = gethex(buf, col);
         if((height < 1) || (srcy + height > 200))
            continue;
         x = gethex(buf, col);
         if((x < 0) || (x > 4096))
            continue;
   ...
   try
   {
      switch(msg->type)
      {
         ...
         case UIM_KEYDOWN: {
            ...
            else if((key = keyxlate.translate(msg->p1, keyboardMode, TYPEMSCANL)) != 0)
            {
            ...
            else if((key = keyxlate.translate(msg->p1, keyboardMode, TYPEMSCANR)) != 0)
            {
...
{
   if(msgStackLen >= maxMessageStack - 1)
      UI_Die(0x7db6);
...
i32 UI_MessageBox(const char *msg, const char *title, i32 flags)
{
   ...
   if(flags & MESSAGE_YESNO)
      MB_FLAGS |= MB_YESNO;
   if(flags & MESSAGE_ICONERROR)
      MB_FLAGS |= MB_ICONERROR;
   if(flags & MESSAGE_ICONWARNING)
...
ui8 CLSID_DS[] =
    {
        0x46, 0xD9, 0xD4, 0x47, 0xE8, 0x62, 0xCF, 0x11, 0x93, 0xBC, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00};

ui8 IID_DS[] =
    {
        0x83, 0xfa, 0x9a, 0x27, 0x81, 0x49, 0xce, 0x11, 0xa5, 0x21, 0x00, 0x20, 0xaf, 0x0b, 0xe5, 0x60};
...
bool MYDIRECTSOUND::Play(const BYTE *wave, i32 attenuation)
{
   ...
   if(wave == NULL)
      return true;
   if(m_numBuffersActive == 64)
      return true; // discard the wave
...
bool UI_PlaySound(const BYTE *wave, i32 /*flags*/, i32 attenuation)
{
   ...
   ASSERT(wave != NULL);
   if(usingDirectX && playDirectSound.Play(wave, attenuation))
      return true;
   if(!usingDirectX)
      playDirectSound.Shutdown();
#else
   if(usingDirectX && myDirectSound.Play(wave, attenuation))
      return true;
   ...
   if(currentTimeInt < finishTime)
      return false; // Wait for previous sound to finish
   sndflags = 0;
...
void UI_PlaySound(char *wave, i32 flags)
{
   ...
   if(flags & SOUND_SYNC)
      sndflags |= SND_SYNC;
   if(flags & SOUND_ASYNC)
      sndflags |= SND_ASYNC;
   if(flags & SOUND_MEMORY)
      sndflags |= SND_MEMORY;
   sndflags |= SND_ASYNC;
...
bool UI_ProcessOption(char *key, char *value)
{
   ...
   if(strcmp(key, "SPEED") == 0)
   {
      ...
      csbMessage.p2 = -1;
      if(strcmp(value, "GLACIAL") == 0)
         csbMessage.p2 = SPEED_GLACIAL;
      if(strcmp(value, "MOLASSES") == 0)
         csbMessage.p2 = SPEED_MOLASSES;
      if(strcmp(value, "VERYSLOW") == 0)
...
void UI_free(void *buf)
{
   ...
   if(*(i32 *)((ui8 *)buf + 2 * sizeof(void *)) != 0xffff)
      listLength--;
   free(buf);
#else
   free(buf);
