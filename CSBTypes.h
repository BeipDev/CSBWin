...
class RecordName // Record Name
{
   ...
   //  friend i16 TAG0093d4(RecordName);
...
private:
   ...
   // } d;
   ui16 indirectIndex;
   RecordName(ui32 pos, ui32 db, ui32 idx);
   bool operator>=(RNVAL i) const
   {
      ASSERT(i >= 0xff00, "Illegal RecordName");
...
public:
   RecordName() { indirectIndex = 0xffff; };
   //{
   ...
   //};
   RecordName(RNVAL i);
   RecordName(DBTYPE dbType, ui32 dbIndex);
   ...
   DBTYPE dbType() const
   {
      if(!checkIndirectIndex())
         return dbUNKNOWN;
      if(dataIndexMap[indirectIndex] == 0xffff)
         return dbUNKNOWN;
   ...
   RecordName pos(ui32 p)
   {
      checkIndirectIndex();
      dataTypeMap[indirectIndex] = (ui8)((dataTypeMap[indirectIndex] & 0xcf) | ((p & 3) << 4));
      return *this;
   };
   ui16 idx() const
   {
   ...
   bool operator==(RecordName rn) const
   {
   ...
   bool operator==(RNVAL i) const
   {
      ASSERT((i >= 0xff00) || (i == RNempty), "Illegal RecordName");
      return indirectIndex == i;
   };
   bool IsSameObjectAs(RecordName obj) const
   {
      return indirectIndex == obj.indirectIndex;
   };
   bool operator!=(RNVAL i) const
   {
      ASSERT((i >= 0xff00) || (i == 0), "Illegal RecordName");
      return indirectIndex != i;
   };
...
void DebugLoad(void *, i32, i32, RecordName);
